...
class reportlib_test extends \advanced_testcase {
    public function test_realtimequiz_report_index_by_keys() {
        $datum = [];
        $object = new \stdClass();
        $object->qid = 3;
        $object->aid = 101;
        ...
        $indexed = realtimequiz_report_index_by_keys($datum, ['aid', 'qid']);
        ...
        $indexed = realtimequiz_report_index_by_keys($datum, ['aid', 'qid'], false);
    ...
    public function test_realtimequiz_report_scale_summarks_as_percentage() {
        $realtimequiz = new \stdClass();
        $realtimequiz->sumgrades = 10;
        $realtimequiz->decimalpoints = 2;

        $this->assertEquals('12.34567%',
            realtimequiz_report_scale_summarks_as_percentage(1.234567, $realtimequiz, false));
        $this->assertEquals('12.35%',
            realtimequiz_report_scale_summarks_as_percentage(1.234567, $realtimequiz, true));
        $this->assertEquals('-',
            realtimequiz_report_scale_summarks_as_percentage('-', $realtimequiz, true));
    }

    public function test_realtimequiz_report_qm_filter_select_only_one_attempt_allowed() {
        $realtimequiz = new \stdClass();
        $realtimequiz->attempts = 1;
        $this->assertSame('', realtimequiz_report_qm_filter_select($realtimequiz));
    }

    public function test_realtimequiz_report_qm_filter_select_average() {
        $realtimequiz = new \stdClass();
        $realtimequiz->attempts = 10;
        $realtimequiz->grademethod = QUIZ_GRADEAVERAGE;
        $this->assertSame('', realtimequiz_report_qm_filter_select($realtimequiz));
    }

    public function test_realtimequiz_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();

        $fakeattempt = new \stdClass();
        $fakeattempt->userid = 123;
        $fakeattempt->realtimequiz = 456;
        $fakeattempt->layout = '1,2,0,3,4,0,5';
        $fakeattempt->state = realtimequiz_attempt::FINISHED;

        // We intentionally insert these in a funny order, to test the SQL better.
        // The test data is:
        // id | realtimequizid | user | attempt | sumgrades | state
        // ---------------------------------------------------
        ...
        $DB->insert_record('realtimequiz_attempts', $fakeattempt);
        ...
        $fakeattempt->state = realtimequiz_attempt::IN_PROGRESS;
        $DB->insert_record('realtimequiz_attempts', $fakeattempt);
        ...
        $realtimequiz->grademethod = QUIZ_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {realtimequiz_attempts} realtimequiza WHERE userid = ? AND realtimequiz = ? AND "
                        . realtimequiz_report_qm_filter_select($realtimequiz), [123, 456]);
        $this->assertEquals(1, count($firstattempt));
        $firstattempt = reset($firstattempt);
        $this->assertEquals(1, $firstattempt->attempt);

        $realtimequiz->grademethod = QUIZ_ATTEMPTLAST;
        ...
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {realtimequiz_attempts} qa_alias WHERE userid = ? AND realtimequiz = ? AND "
                . realtimequiz_report_qm_filter_select($realtimequiz, 'qa_alias'), [123, 456]);
