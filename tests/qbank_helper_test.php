...
/**
 * Qbank helper test for realtimequiz.
 *
...
class qbank_helper_test extends \advanced_testcase {
    ...
    public function test_reference_records() {
        ...
        $this->assertEquals($question->id, qbank_helper::choose_question_for_redo(
                $realtimequiz->id, $context, $slot->id, new \qubaid_list([])));
    ...
    public function test_get_question_structure() {
        ...
        $realtimequiz = $this->create_test_realtimequiz($this->course);
        $realtimequizcontext = \context_module::instance(get_coursemodule_from_instance("realtimequiz", $realtimequiz->id, $this->course->id)->id);

        // Create a question in the realtimequiz question bank.
        /** @var \core_question_generator $questiongenerator */
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category(['contextid' => $realtimequizcontext->id]);
        ...
        realtimequiz_add_realtimequiz_question($q->id, $realtimequiz);

        // Load the realtimequiz object and check.
        $realtimequizobj = \mod_realtimequiz\realtimequiz_settings::create($realtimequiz->id);
        $realtimequizobj->preload_questions();
        $realtimequizobj->load_questions();
        $questions = $realtimequizobj->get_questions();
    ...
    public function test_get_question_structure_with_drafts(): void {
        ...
        // Create some questions with drafts in the realtimequiz question bank.
        /** @var \core_question_generator $questiongenerator */
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category(['contextid' => $realtimequizcontext->id]);
        ...
        // Add the questions to the realtimequiz.
        realtimequiz_add_realtimequiz_question($q1->id, $realtimequiz);
        realtimequiz_add_realtimequiz_question($q2->id, $realtimequiz);
        realtimequiz_add_realtimequiz_question($q3->id, $realtimequiz);
