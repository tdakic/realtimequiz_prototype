...
/**
 * Unit tests restoring realtimequiz attempts
 *
...
class restore_attempt_test extends \advanced_testcase {
     ...
     * Test restore dates.
     *
     * @covers \restore_realtimequiz_activity_structure_step
     */
    public function test_restore_question_attempts_missing_users(): void {
        ...
        // Contains a realtimequiz with four attempts from different users. The users are as follows (user ID -> user):
        ...
        $restoredrealtimequiz = $DB->get_record('realtimequiz', []);
        ...
        $this->assertContains("Mapped user ID not found for user 4, realtimequiz {$restoredrealtimequiz->id}, attempt 1. Skipping realtimequiz attempt",
            $loginfomessages);
        $this->assertContains("Mapped user ID not found for user 5, realtimequiz {$restoredrealtimequiz->id}, attempt 1. Skipping realtimequiz attempt",
            $loginfomessages);

        // User 01 has supplied the wrong answer, assert dates match the backup file too.
        $user01attempt = $DB->get_record('realtimequiz_attempts', [
            'realtimequiz' => $restoredrealtimequiz->id,
        ...
        $user04attempt = $DB->get_record('realtimequiz_attempts', [
            'realtimequiz' => $restoredrealtimequiz->id,
