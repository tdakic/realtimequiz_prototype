...
class structure_test extends \advanced_testcase {
    ...
    /**
     * Create a course with an empty realtimequiz.
     * @return array with three elements realtimequiz, cm and course.
     */
    protected function prepare_realtimequiz_data() {
        ...
        return [$realtimequiz, $cm, $course];
    }

    /**
     * Creat a test realtimequiz.
     *
     ...
     * @return realtimequiz_settings the created realtimequiz.
     */
    protected function create_test_realtimequiz($layout) {
        list($realtimequiz, $cm, $course) = $this->prepare_realtimequiz_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                realtimequiz_add_realtimequiz_question($q->id, $realtimequiz, $page);
                $lastpage = $page;
            }
        }
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_realtimequiz()}.
     * @param structure $structure the structure to test.
     */
    protected function assert_realtimequiz_layout($expectedlayout, structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first realtimequiz section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The realtimequiz contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the realtimequiz.');
            }
        }
    }
     ...
     * used by create_test_realtimequiz as assert_realtimequiz_layout.
    ...
    public function test_get_realtimequiz_slots() {
        $realtimequizobj = $this->create_test_realtimequiz([
                ['TF1', 1, 'truefalse'],
                ['TF2', 1, 'truefalse'],
        ]);
        $structure = structure::create_for_realtimequiz($realtimequizobj);
    ...
    public function test_realtimequiz_has_one_section_by_default() {
        $realtimequizobj = $this->create_test_realtimequiz([
                ['TF1', 1, 'truefalse'],
        ]);
        $structure = structure::create_for_realtimequiz($realtimequizobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = structure::create_for_realtimequiz($realtimequizobj);
        $this->assert_realtimequiz_layout([
                'Heading 1',
                ['TF1', 1, 'truefalse'],
                ['TF2', 2, 'truefalse'],
        ], $structure);
    ...
    public function test_realtimequiz_remove_slot() {
        $realtimequizobj = $this->create_test_realtimequiz([
                ['TF1', 1, 'truefalse'],
                ['TF2', 1, 'truefalse'],
                'Heading 2',
    ...
    public function test_realtimequiz_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        realtimequiz_add_random_questions($realtimequizobj->get_realtimequiz(), 1, $cat->id, 1, false);
        $structure = structure::create_for_realtimequiz($realtimequizobj);
        $sql = 'SELECT qsr.*
                 FROM {question_set_references} qsr
                 JOIN {realtimequiz_slots} qs ON qs.id = qsr.itemid
                 WHERE qs.realtimequizid = ?
                   AND qsr.component = ?
                   AND qsr.questionarea = ?';
        $randomq = $DB->get_record_sql($sql, [$realtimequizobj->get_realtimequizid(), 'mod_realtimequiz', 'slot']);
        ...
        $this->assertFalse($DB->record_exists('question_set_references',
            ['id' => $randomq->id, 'component' => 'mod_realtimequiz', 'questionarea' => 'slot']));
    }
    ...
    public function test_can_remove_last_question_in_a_realtimequiz() {
        $realtimequizobj = $this->create_test_realtimequiz([
                'Heading 1',
                ['TF1', 1, 'truefalse'],
        ]);
        ...
        realtimequiz_add_realtimequiz_question($q->id, $realtimequizobj->get_realtimequiz(), 0);
        $structure = structure::create_for_realtimequiz($realtimequizobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        realtimequiz_add_realtimequiz_question($q->id, $realtimequizobj->get_realtimequiz(), 1);
    ...
    public function test_can_add_random_questions() {
        ...
        $realtimequiz = $this->create_test_realtimequiz([]);
        $course = $realtimequiz->get_course();
        ...
        $structure = structure::create_for_realtimequiz($realtimequiz);
    ...
    public function test_get_version_choices_for_slot() {
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category(['contextid' => $realtimequizobj->get_context()->id]);
        ...
        realtimequiz_add_realtimequiz_question($q->id, $realtimequizobj->get_realtimequiz());
    ...
    public function test_has_use_capability() {
        ...
        // Create a realtimequiz with question.
        $realtimequizobj = $this->create_test_realtimequiz([]);
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category(['contextid' => $realtimequizobj->get_context()->id]);
        $q = $questiongenerator->create_question('essay', null,
