...
Feature: Allow students to redo questions in a practice realtimequiz, without starting a whole new attempt
  In order to practice particular skills I am struggling with
  As a student
  I need to be able to redo each question in a realtimequiz as often as necessary without starting a whole new attempt, if my teacher allows it.
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | realtimequiz     | Quiz 1 | Quiz 1 description | C1     | realtimequiz1    | immediatefeedback  | 1                |
  ...
  Scenario: After completing a question, there is a redo question button that restarts the question
    Given I am on the "Quiz 1" "mod_realtimequiz > View" page logged in as "student"
  ...
  Scenario: After redoing a question, regrade works
    ...
    And I click on "Check" "button" in the "First question" "question"
    And I press "Try another question like this one"
    And I am on the "Quiz 1" "mod_realtimequiz > Grades report" page logged in as "teacher"
  ...
  Scenario: The redo question button is visible but disabled for teachers
    ...
    And I click on "Check" "button" in the "First question" "question"
    And I log out
    And I am on the "Quiz 1" "mod_realtimequiz > View" page logged in as "teacher"
  ...
  Scenario: Teachers reviewing can see all the questions attempted in a slot
    ...
    And I am on the "Quiz 1" "mod_realtimequiz > Statistics report" page logged in as teacher
    And I follow "TF1"
    And "False" row "Frequency" column of "realtimequizresponseanalysis" table should contain "100.00%"
    And "True" row "Frequency" column of "realtimequizresponseanalysis" table should contain "0.00%"
    And "[No response]" row "Frequency" column of "realtimequizresponseanalysis" table should contain "100.00%"
  ...
  Scenario: Teachers reviewing can switch between attempts in the review question popup
    ...
    # Now switch to the other realtimequiz attempt using the link at the top, which does not have a redo.
  ...
  Scenario: Redoing questions should work with random questions as well
    Given the following "activities" exist:
      | activity | name   | intro              | course | idnumber | preferredbehaviour | canredoquestions |
      | realtimequiz     | Quiz 2 | Quiz 2 description | C1     | realtimequiz2    | immediatefeedback  | 1                |
    And I am on the "Quiz 2" "mod_realtimequiz > Edit" page logged in as "admin"
    And I open the "last" add to realtimequiz menu
    And I follow "a random question"
    And I press "Add random question"
    And user "student" has started an attempt at realtimequiz "Quiz 2" randomised as follows:
      | slot | actualquestion |
      | 1    | TF1            |
    And I am on the "Quiz 2" "mod_realtimequiz > View" page logged in as "student"
  ...
  Scenario: Teachers reviewing can see author of action in review attempt
    ...
    And the following "activities" exist:
      ...
      | realtimequiz     | Quiz 2 | Quiz 2 description | C1     | realtimequiz2    |
    ...
    And user "student" has attempted "Quiz 2" with responses:
      | slot | response    |
      | 1    | J.K.Rowling |
    And I am on the "Quiz 2" "mod_realtimequiz > Manual grading report" page logged in as "teacher"
