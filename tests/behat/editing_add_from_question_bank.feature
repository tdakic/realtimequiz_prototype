...
Feature: Adding questions to a realtimequiz from the question bank
  ...
  Scenario: Questions are added in the right place with multiple sections
    ...
    And I log in as "teacher1"
    And I am on the "Quiz 1" "mod_realtimequiz > Edit" page
    When I open the "Page 1" add to realtimequiz menu
    And I follow "from question bank"
    And I set the field with xpath "//tr[contains(normalize-space(.), 'question 03 name')]//input[@type='checkbox']" to "1"
    And I click on "Add selected questions to the realtimequiz" "button"
    Then I should see "question 03 name" on realtimequiz page "1"
    And I should see "question 01 name" before "question 03 name" on the edit realtimequiz page
  ...
  Scenario: Add several selected questions from the question bank
    ...
    And I press "Add selected questions to the realtimequiz"
    Then I should see "question 01 name" on realtimequiz page "1"
    And I should see "question 02 name" on realtimequiz page "2"
  ...
  Scenario: Shuffle option could be set before adding any question to the realtimequiz
    Given the following "questions" exist:
      | questioncategory | qtype | name             | questiontext     |
      | Test questions   | essay | question 03 name | question 03 text |
    And I log in as "teacher1"
