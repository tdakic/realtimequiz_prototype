...
/**
 * This page handles editing and creation of realtimequiz overrides
 *
...
$url = new moodle_url('/mod/realtimequiz/overrideedit.php');
if ($action) {
...
// Merge realtimequiz defaults with data.
$keys = ['timeopen', 'timeclose', 'timelimit', 'attempts', 'password'];
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $realtimequiz->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/realtimequiz/overrides.php', ['cmid' => $cm->id]);
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new edit_override_form($url, $cm, $realtimequiz, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->realtimequiz = $realtimequiz->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $realtimequiz->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('realtimequiz_overrides', $conditions)) {
            ...
            realtimequiz_delete_override($realtimequiz, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('realtimequiz_overrides', $fromform);
        $cachekey = $groupmode ? "{$fromform->realtimequiz}_g_{$fromform->groupid}" : "{$fromform->realtimequiz}_u_{$fromform->userid}";
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('realtimequiz_overrides', $fromform);
        $cachekey = $groupmode ? "{$fromform->realtimequiz}_g_{$fromform->groupid}" : "{$fromform->realtimequiz}_u_{$fromform->userid}";
        cache::make('mod_realtimequiz', 'overrides')->delete($cachekey);
    ...
    if ($groupmode) {
        ...
        realtimequiz_update_events($realtimequiz, $fromform);
    }
...
// Print the form.
$pagetitle = get_string('editoverride', 'realtimequiz');
