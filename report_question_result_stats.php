...
/* the below was needed until laziness took over
$context = context_module::instance($cm->id);
echo $context->id;

$currentgroup = $report->get_current_group($cm, $course, $context);
$groupstudentsjoins = get_enrolled_with_capabilities_join($context, '',['mod/realtimequiz:reviewmyattempts', 'mod/realtimequiz:attempt'], $currentgroup);
$whichattempts = question_attempt::LAST_TRY;

$qubaids = realtimequiz_statistics_qubaids_condition($realtimequiz->id, $groupstudentsjoins, $whichattempts);
$questions = $report->load_and_initialise_questions_for_calculations($realtimequiz);

//$report->output_individual_question_response_analysis(($question, $variantno, $s, $reporturl, $qubaids));
