 ...
 */
class backup_realtimequiz_activity_structure_step extends backup_questions_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $this->add_subplugin_structure('realtimequizaccess', $realtimequiz, true);

        $qinstances = new backup_nested_element('question_instances');

        $qinstance = new backup_nested_element('question_instance', ['id'],
                ['realtimequizid', 'slot', 'page', 'displaynumber', 'requireprevious', 'maxmark']);

        $this->add_question_references($qinstance, 'mod_realtimequiz', 'slot');

        $this->add_question_set_references($qinstance, 'mod_realtimequiz', 'slot');
        ...
        $this->add_subplugin_structure('realtimequizaccess', $attempt, true);

        // Build the tree.
        $realtimequiz->add_child($qinstances);
        $qinstances->add_child($qinstance);

        $realtimequiz->add_child($sections);
        $sections->add_child($section);

        $realtimequiz->add_child($feedbacks);
        $feedbacks->add_child($feedback);

        $realtimequiz->add_child($overrides);
        $overrides->add_child($override);

        $realtimequiz->add_child($grades);
        $grades->add_child($grade);

        $realtimequiz->add_child($attempts);
        $attempts->add_child($attempt);

        // Define sources.
        $realtimequiz->set_source_table('realtimequiz', ['id' => backup::VAR_ACTIVITYID]);

        $qinstance->set_source_table('realtimequiz_slots', ['realtimequizid' => backup::VAR_PARENTID]);

        $section->set_source_table('realtimequiz_sections', ['realtimequizid' => backup::VAR_PARENTID]);

        $feedback->set_source_table('realtimequiz_feedback', ['realtimequizid' => backup::VAR_PARENTID]);

        // Quiz overrides to backup are different depending of user info.
        $overrideparams = ['realtimequiz' => backup::VAR_PARENTID];
        ...
        $override->set_source_table('realtimequiz_overrides', $overrideparams);

        // All the rest of elements only happen if we are including user info.
        if ($userinfo) {
            $grade->set_source_table('realtimequiz_grades', ['realtimequiz' => backup::VAR_PARENTID]);
            $attempt->set_source_sql('
                    SELECT *
                    FROM {realtimequiz_attempts}
                    WHERE realtimequiz = :realtimequiz AND preview = 0', ['realtimequiz' => backup::VAR_PARENTID]);
        }

        // Define source alias.
        $realtimequiz->set_source_alias('attempts', 'attempts_number');
        ...
        $feedback->annotate_files('mod_realtimequiz', 'feedback', 'id');
