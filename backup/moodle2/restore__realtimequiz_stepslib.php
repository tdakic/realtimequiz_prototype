 ...
 */
class restore_realtimequiz_activity_structure_step extends restore_questions_activity_structure_step {

    /**
     * @var bool tracks whether the realtimequiz contains at least one section. Before
     * Moodle 2.9 realtimequiz sections did not exist, so if the file being restored
     * did not contain any, we need to create one in {@link after_execute()}.
     */
    protected $sectioncreated = false;

    /** @var stdClass|null $currentrealtimequizattempt Track the current realtimequiz attempt being restored. */
    protected $currentrealtimequizattempt = null;

    /**
     * @var bool when restoring old realtimequizzes (2.8 or before) this records the
     * shufflequestionsoption realtimequiz option which has moved to the realtimequiz_sections table.
     */
    protected $legacyshufflequestionsoption = false;

    /** @var stdClass */
    protected $oldrealtimequizlayout;
    ...
    protected function define_structure() {
        ...
        $realtimequiz = new restore_path_element('realtimequiz', '/activity/realtimequiz');
        $paths[] = $realtimequiz;

        // A chance for access subplugings to set up their realtimequiz data.
        $this->add_subplugin_structure('realtimequizaccess', $realtimequiz);

        $realtimequizquestioninstance = new restore_path_element('realtimequiz_question_instance',
            '/activity/realtimequiz/question_instances/question_instance');
        $paths[] = $realtimequizquestioninstance;
        if ($this->task->get_old_moduleversion() < 2021091700) {
            $paths[] = new restore_path_element('realtimequiz_slot_tags',
                '/activity/realtimequiz/question_instances/question_instance/tags/tag');
        } else {
            $this->add_question_references($realtimequizquestioninstance, $paths);
            $this->add_question_set_references($realtimequizquestioninstance, $paths);
        }
        $paths[] = new restore_path_element('realtimequiz_section', '/activity/realtimequiz/sections/section');
        $paths[] = new restore_path_element('realtimequiz_feedback', '/activity/realtimequiz/feedbacks/feedback');
        $paths[] = new restore_path_element('realtimequiz_override', '/activity/realtimequiz/overrides/override');

        if ($userinfo) {
            $paths[] = new restore_path_element('realtimequiz_grade', '/activity/realtimequiz/grades/grade');

            if ($this->task->get_old_moduleversion() > 2011010100) {
                // Restoring from a version 2.1 dev or later.
                // Process the new-style attempt data.
                $realtimequizattempt = new restore_path_element('realtimequiz_attempt',
                        '/activity/realtimequiz/attempts/attempt');
                $paths[] = $realtimequizattempt;

                // Add states and sessions.
                $this->add_question_usages($realtimequizattempt, $paths);

                // A chance for access subplugings to set up their attempt data.
                $this->add_subplugin_structure('realtimequizaccess', $realtimequizattempt);

            } else {
                // Restoring from a version 2.0.x+ or earlier.
                // Upgrade the legacy attempt data.
                $realtimequizattempt = new restore_path_element('realtimequiz_attempt_legacy',
                        '/activity/realtimequiz/attempts/attempt',
                        true);
                $paths[] = $realtimequizattempt;
                $this->add_legacy_question_attempt_data($realtimequizattempt, $paths);
            }
        }
    ...
    protected function process_realtimequiz($data) {
        ...
        if (property_exists($data, 'questions')) {
            // Needed by {@link process_realtimequiz_attempt_legacy}, in which case it will be present.
            $this->oldrealtimequizlayout = $data->questions;
        }

        // The setting realtimequiz->attempts can come both in data->attempts and
        ...
        if (!isset($data->browsersecurity)) {
            if (empty($data->popup)) {
                ...
                // Since 3.9 realtimequizaccess_safebrowser replaced with a new realtimequizaccess_seb.
                $data->browsersecurity = '-';
                $addsebrule = true;
            } else {
        ...
        if (!isset($data->overduehandling)) {
            $data->overduehandling = get_config('realtimequiz', 'overduehandling');
        }

        // Old shufflequestions setting is now stored in realtimequiz sections,
        ...
        // Insert the realtimequiz record.
        ...
        if (!empty($addsebrule)) {
            ...
            $sebsettings->realtimequizid = $newitemid;
    ...
    /**
     * Process the data for pre 4.0 realtimequiz data where the question_references and question_set_references table introduced.
     *
     * @param stdClass|array $data
     */
    protected function process_realtimequiz_question_legacy_instance($data) {
        global $DB;
        ...
        $module = $DB->get_record('realtimequiz', ['id' => $data->realtimequizid]);
        ...
        if ($question->qtype === 'random') {
            ...
            $questionsetreference->usingcontextid = context_module::instance(get_coursemodule_from_instance(
                "realtimequiz", $module->id, $module->course)->id)->id;
            $questionsetreference->component = 'mod_realtimequiz';
            ...
            $questionreference->usingcontextid = context_module::instance(get_coursemodule_from_instance(
                "realtimequiz", $module->id, $module->course)->id)->id;
            $questionreference->component = 'mod_realtimequiz';
    ...
    /**
     * Process realtimequiz slots.
     *
     * @param stdClass|array $data
     */
    protected function process_realtimequiz_question_instance($data) {
        global $CFG, $DB;
        ...
        if (!property_exists($data, 'slot')) {
            $page = 1;
            $slot = 1;
            foreach (explode(',', $this->oldrealtimequizlayout) as $item) {
                if ($item == 0) {
                    $page += 1;
                    continue;
                }
        ...
        if (!property_exists($data, 'slot')) {
            ...
            // that was not actually in the realtimequiz. Drop it.
            $this->log('question ' . $data->questionid . ' was associated with realtimequiz ' .
                    $this->get_new_parentid('realtimequiz') . ' but not actually used. ' .
                    'The instance has been ignored.', backup::LOG_INFO);
            return;
        }

        $data->realtimequizid = $this->get_new_parentid('realtimequiz');

        $newitemid = $DB->insert_record('realtimequiz_slots', $data);
        // Add mapping, restore of slot tags (for random questions) need it.
        $this->set_mapping('realtimequiz_question_instance', $oldid, $newitemid);

        if ($this->task->get_old_moduleversion() < 2022020300) {
            $data->id = $newitemid;
            $this->process_realtimequiz_question_legacy_instance($data);
        }
    }

    /**
     * Process a realtimequiz_slot_tags to restore the tags to the new structure.
     *
     * @param stdClass|array $data The realtimequiz_slot_tags data
     */
    protected function process_realtimequiz_slot_tags($data) {
        global $DB;

        $data = (object) $data;
        $slotid = $this->get_new_parentid('realtimequiz_question_instance');
        ...
        $setreferencedata = $DB->get_record('question_set_references',
            ['itemid' => $slotid, 'component' => 'mod_realtimequiz', 'questionarea' => 'slot']);
    ...
    protected function process_realtimequiz_section($data) {
        global $DB;

        $data = (object) $data;
        $data->realtimequizid = $this->get_new_parentid('realtimequiz');
        $oldid = $data->id;
        $newitemid = $DB->insert_record('realtimequiz_sections', $data);
        $this->sectioncreated = true;
        $this->set_mapping('realtimequiz_section', $oldid, $newitemid, true);
    }

    protected function process_realtimequiz_feedback($data) {
        global $DB;
        ...
        $newitemid = $DB->insert_record('realtimequiz_feedback', $data);
        $this->set_mapping('realtimequiz_feedback', $oldid, $newitemid, true); // Has related files.
    }

    protected function process_realtimequiz_override($data) {
        global $DB;
        ...
        $data->realtimequiz = $this->get_new_parentid('realtimequiz');
        ...
        $newitemid = $DB->insert_record('realtimequiz_overrides', $data);

        // Add mapping, restore of logs needs it.
        $this->set_mapping('realtimequiz_override', $oldid, $newitemid);
    }

    protected function process_realtimequiz_grade($data) {
        global $DB;
        ...
        $DB->insert_record('realtimequiz_grades', $data);
    }

    protected function process_realtimequiz_attempt($data) {
        $data = (object)$data;

        $data->realtimequiz = $this->get_new_parentid('realtimequiz');
        $data->attempt = $data->attemptnum;

        // Get user mapping, return early if no mapping found for the realtimequiz attempt.
        $olduserid = $data->userid;
        $data->userid = $this->get_mappingid('user', $olduserid, 0);
        if ($data->userid === 0) {
            $this->log('Mapped user ID not found for user ' . $olduserid . ', realtimequiz ' . $this->get_new_parentid('realtimequiz') .
                ', attempt ' . $data->attempt . '. Skipping realtimequiz attempt', backup::LOG_INFO);

            $this->currentrealtimequizattempt = null;
            return;
        }
        ...
        $this->currentrealtimequizattempt = clone($data);
    }

    protected function process_realtimequiz_attempt_legacy($data) {
        global $DB;

        $this->process_realtimequiz_attempt($data);

        $realtimequiz = $DB->get_record('realtimequiz', ['id' => $this->get_new_parentid('realtimequiz')]);
        $realtimequiz->oldquestions = $this->oldrealtimequizlayout;
        $this->process_legacy_realtimequiz_attempt_data($data, $realtimequiz);
    }

    protected function inform_new_usage_id($newusageid) {
        global $DB;

        $data = $this->currentrealtimequizattempt;
        if ($data === null) {
            return;
        }
        ...
        $newitemid = $DB->insert_record('realtimequiz_attempts', $data);

        // Save realtimequiz_attempt->id mapping, because logs use it.
        $this->set_mapping('realtimequiz_attempt', $oldid, $newitemid, false);
    }
    ...
    protected function after_execute() {
        ...
        // Add realtimequiz related files, no need to match by itemname (just internally handled context).
        $this->add_related_files('mod_realtimequiz', 'intro', null);
        // Add feedback related files, matching by itemname = 'realtimequiz_feedback'.
        $this->add_related_files('mod_realtimequiz', 'feedback', 'realtimequiz_feedback');

        if (!$this->sectioncreated) {
            $DB->insert_record('realtimequiz_sections', [
