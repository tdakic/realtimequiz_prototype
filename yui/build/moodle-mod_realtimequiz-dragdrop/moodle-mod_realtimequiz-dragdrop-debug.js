YUI.add('moodle-mod_realtimequiz-dragdrop', function (Y, NAME) {

/* eslint-disable no-unused-vars */
/**
 * Drag and Drop for Quiz sections and slots.
 *
 * @module moodle-mod-realtimequiz-dragdrop
 */
...
var CSS = {
    ...
    CONTENT: 'content',
    COURSECONTENT: 'mod-realtimequiz-edit-content',
 ...
 *
 * @class M.mod_realtimequiz.dragdrop.section
 * @constructor
...
Y.extend(DRAGSECTION, M.core.dragdrop, {
    ...
    initializer: function() {
        ...
        if (this.sectionlistselector) {
            ...
            del.dd.plug(Y.Plugin.DDConstrained, {
                // Keep it inside the .mod-realtimequiz-edit-content
    ...
    drop_hit: function(e) {
        ...
        if (dragnodeid === dropnodeindex) {
            Y.log("Skipping move - same location moving " + dragnodeid + " to " + dropnodeindex,
                  'debug', 'moodle-mod_realtimequiz-dragdrop');
            return;
        }

        Y.log("Moving from position " + dragnodeid + " to position " + dropnodeindex, 'debug', 'moodle-mod_realtimequiz-dragdrop');
        ...
        params.realtimequizid = this.get('realtimequizid');
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    try {
                        ...
                        M.mod_realtimequiz.edit.process_sections(Y, sectionlist, responsetext, loopstart, loopend);
                    } catch (e) {
                        // Ignore.
                    }
                    ...
                    do {
                        ...
                        for (index = loopstart; index <= loopend; index++) {
                            if (Y.Moodle.core_course.util.section.getId(sectionlist.item(index - 1)) >
                                ...
                                M.mod_realtimequiz.edit.swap_sections(Y, index - 1, index);
    ...
    NAME: 'mod_realtimequiz-dragdrop-section',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_realtimequiz.init_section_dragdrop = function(params) {
...
Y.extend(DRAGRESOURCE, M.core.dragdrop, {
    initializer: function() {
        ...
        M.mod_realtimequiz.dragres = this;
    },
    ...
    setup_for_section: function() {
        Y.Node.all('.mod-realtimequiz-edit-content ul.slots ul.section').each(function(resources) {
    ...
    drop_hit: function(e) {
        ...
        params.field = 'move';
        params.id = Number(Y.Moodle.mod_realtimequiz.util.slot.getId(dragnode));
        ...
        if (previousslot) {
            params.previousid = Number(Y.Moodle.mod_realtimequiz.util.slot.getId(previousslot));
        }

        var previouspage = dragnode.previous(SELECTOR.PAGE);
        if (previouspage) {
            params.page = Number(Y.Moodle.mod_realtimequiz.util.page.getId(previouspage));
        }
        ...
        Y.io(uri, {
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    M.mod_realtimequiz.realtimequizbase.invoke_function('set_visibility_resource_ui', params);
                    this.unlock_drag_handle(drag, CSS.EDITINGMOVE);
                    window.setTimeout(function() {
                        spinner.hide();
                    }, 250);
                    M.mod_realtimequiz.resource_toolbox.reorganise_edit_page();
                },
                failure: function(tid, response) {
                    this.ajax_failure(response);
    ...
    NAME: 'mod_realtimequiz-dragdrop-resource',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_realtimequiz = M.mod_realtimequiz || {};
M.mod_realtimequiz.init_resource_dragdrop = function(params) {
    ...
    "requires": [
        ...
        "moodle-mod_realtimequiz-util-base",
        "moodle-mod_realtimequiz-util-page",
