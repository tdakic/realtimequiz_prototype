YUI.add('moodle-mod_realtimequiz-autosave', function (Y, NAME) {
...
/**
 * Auto-save functionality for during realtimequiz attempts.
 *
 * @module moodle-mod_realtimequiz-autosave
 */

/**
 * Auto-save functionality for during realtimequiz attempts.
 *
 * @class M.mod_realtimequiz.autosave
 */

M.mod_realtimequiz = M.mod_realtimequiz || {};
M.mod_realtimequiz.autosave = {
    /**
     ...
     * @type String
     * @default M.cfg.wwwroot + '/mod/realtimequiz/autosave.ajax.php'
     * @private
     */
    AUTOSAVE_HANDLER: M.cfg.wwwroot + '/mod/realtimequiz/autosave.ajax.php',
    ...
    init: function(delay) {
        ...
        if (!this.form) {
            Y.log('No response form found. Why did you try to set up autosave?', 'debug', 'moodle-mod_realtimequiz-autosave');
            return;
        }
    ...
    init_tinymce: function(repeatcount) {
        if (typeof window.tinyMCE === 'undefined') {
            if (repeatcount > 0) {
                ...
                Y.log('Gave up looking for TinyMCE.', 'debug', 'moodle-mod_realtimequiz-autosave');
            }
            return;
        }

        Y.log('Found TinyMCE.', 'debug', 'moodle-mod_realtimequiz-autosave');
    ...
    init_tinymce_editor: function(e, editor) {
        Y.log('Found TinyMCE editor ' + editor.id + '.', 'debug', 'moodle-mod_realtimequiz-autosave');
    ...
    value_changed: function(e) {
        ...
        Y.log('Detected a value change in element ' + name + '.', 'debug', 'moodle-mod_realtimequiz-autosave');
        this.start_save_timer_if_necessary();
    },

    editor_changed: function(editor) {
        Y.log('Detected a value change in editor ' + editor.id + '.', 'debug', 'moodle-mod_realtimequiz-autosave');
        this.start_save_timer_if_necessary();
    },
    ...
    save_changes: function() {
        ...
        if (this.is_time_nearly_over()) {
            Y.log('No more saving, time is nearly over.', 'debug', 'moodle-mod_realtimequiz-autosave');
            this.stop_autosaving();
            return;
        }

        Y.log('Doing a save.', 'debug', 'moodle-mod_realtimequiz-autosave');
        if (typeof window.tinyMCE !== 'undefined') {
    ...
    save_done: function(transactionid, response) {
        ...
        if (typeof autosavedata.timeleft !== 'undefined') {
            Y.log('Updating timer: ' + autosavedata.timeleft + ' seconds remain.', 'debug', 'moodle-mod_realtimequiz-timer');
            M.mod_realtimequiz.timer.updateEndTime(autosavedata.timeleft);
        }

        Y.log('Save completed.', 'debug', 'moodle-mod_realtimequiz-autosave');
        this.save_transaction = null;

        if (this.dirty) {
            Y.log('Dirty after save.', 'debug', 'moodle-mod_realtimequiz-autosave');
            this.start_save_timer();
        }
    ...
    save_failed: function() {
        Y.log('Save failed.', 'debug', 'moodle-mod_realtimequiz-autosave');
        this.save_transaction = null;
    ...
    is_time_nearly_over: function() {
        return M.mod_realtimequiz.timer && M.mod_realtimequiz.timer.endtime &&
                (new Date().getTime() + 2 * this.delay) > M.mod_realtimequiz.timer.endtime;
    },
