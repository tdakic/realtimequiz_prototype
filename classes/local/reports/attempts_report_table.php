...
abstract class attempts_report_table extends \table_sql {
    ...
    /** @var stdClass the realtimequiz settings for the realtimequiz we are reporting on. */
    ...
    /** @var array the questions that comprise this realtimequiz. */
    protected $questions;
    ...
    protected $togglegroup = 'realtimequiz-attempts';
     ...
     */
    public function __construct($uniqueid, $realtimequiz, $context, $qmsubselect,
    ...
    public function col_checkbox($attempt) {
        ...
        if ($attempt->attempt) {
            $checkbox = new \core\output\checkbox_toggleall($this->togglegroup, false, [
                ...
                'label' => get_string('selectattempt', 'realtimequiz'),
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/realtimequiz/review.php', ['attempt' => $attempt->attempt]),
                get_string('reviewattempt', 'realtimequiz'), ['class' => 'reviewlink']);
    }
    ...
    public function col_state($attempt) {
        ...
        $display = realtimequiz_attempt::state_name($attempt->state);
        if ($this->is_downloading()) {
            return $display;
        }

        $this->canreopen ??= has_capability('mod/realtimequiz:reopenattempts', $this->context);
        if ($attempt->state == realtimequiz_attempt::ABANDONED && $this->canreopen) {
            $display .= ' ' . html_writer::tag('button', get_string('reopenattempt', 'realtimequiz'), [
                'type' => 'button',
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = realtimequiz_report_feedback_for_grade(
                realtimequiz_rescale_grade($attempt->sumgrades, $this->realtimequiz, false),
                $this->realtimequiz->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/realtimequiz/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        ['height' => 450, 'width' => 650]),
                ['title' => get_string('reviewresponse', 'realtimequiz')]);
    ...
    /**
     * Get the question attempt state for a particular question in a particular realtimequiz attempt.
     *
    ...
    public function base_sql(\core\dml\sql_join $allowedstudentsjoins) {
        ...
        // Please note this uniqueid column is not the same as realtimequiza.uniqueid.
        $fields = 'DISTINCT ' . $DB->sql_concat('u.id', "'#'", 'COALESCE(realtimequiza.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                realtimequiza.uniqueid AS usageid,
                realtimequiza.id AS attempt,
                u.id AS userid,
                u.idnumber,
                u.picture,
                u.imagealt,
                ...
                realtimequiza.state,
                realtimequiza.sumgrades,
                realtimequiza.timefinish,
                realtimequiza.timestart,
                CASE WHEN realtimequiza.timefinish = 0 THEN null
                     WHEN realtimequiza.timefinish > realtimequiza.timestart THEN realtimequiza.timefinish - realtimequiza.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and realtimequiz_attempts tables.
        $from = " {user} u";
        $from .= "\n{$userfields->joins}";
        $from .= "\nLEFT JOIN {realtimequiz_attempts} realtimequiza ON
                                    realtimequiza.userid = u.id AND realtimequiza.realtimequiz = :realtimequizid";
        $params = array_merge($userfields->params, ['realtimequizid' => $this->realtimequiz->id]);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (realtimequiza.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = realtimequiz_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'realtimequiza.id IS NOT NULL AND realtimequiza.preview = 0';
                break;
            case attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "realtimequiza.preview = 0 AND realtimequiza.id IS NOT NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "realtimequiza.id IS NULL AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
            case attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                $from .= "\n" . $allowedstudentsjoins->joins;
                $where = "(realtimequiza.preview = 0 OR realtimequiza.preview IS NULL) AND " . $allowedstudentsjoins->wheres;
                $params = array_merge($params, $allowedstudentsjoins->params);
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (realtimequiza.state $statesql OR realtimequiza.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{realtimequiz_attempts} {$alias}realtimequiza", "{$alias}realtimequiza.uniqueid",
                "{$alias}realtimequiza.realtimequiz = :{$alias}realtimequizid", ["{$alias}realtimequizid" => $this->sql->params['realtimequizid']]);
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = realtimequiza.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join("(
                SELECT DISTINCT realtimequiza.uniqueid
                  FROM " . $this->sql->from . "
                 WHERE " . $this->sql->where . "
                    ) realtimequizasubquery", 'realtimequizasubquery.uniqueid',
                    "1 = 1", $this->sql->params);
        }
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['realtimequiza.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/realtimequiz:deleteattempts', $this->context)) {
            $deletebuttonparams = [
                'type'  => 'submit',
                'class' => 'btn btn-secondary mr-1',
                'id'    => 'deleteattemptsbutton',
                'name'  => 'delete',
                'value' => get_string('deleteselected', 'realtimequiz_overview'),
                'data-action' => 'toggle',
            ...
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    ['message' => get_string('deleteattemptcheck', 'realtimequiz')]);
        }
    }
     ...
     *
     * It returns the HTML for a master \core\output\checkbox_toggleall component that selects/deselects all realtimequiz attempts.
     *
    ...
    public function checkbox_col_header(string $columnname) {
        ...
        $selectalltext = get_string('selectall', 'realtimequiz');
        $deselectalltext = get_string('selectnone', 'realtimequiz');
