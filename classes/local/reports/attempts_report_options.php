...
class attempts_report_options {
    ...
    /** @var stdClass the settings for the realtimequiz being reported on. */
    public $realtimequiz;

    /** @var stdClass the course module objects for the realtimequiz being reported on. */
    public $cm;

    /** @var stdClass the course settings for the course the realtimequiz is in. */
    public $course;

    /**
     * @var array form field name => corresponding realtimequiz_attempt:: state constant.
     */
    protected static $statefields = [
        'stateinprogress' => realtimequiz_attempt::IN_PROGRESS,
        'stateoverdue'    => realtimequiz_attempt::OVERDUE,
        'statefinished'   => realtimequiz_attempt::FINISHED,
        'stateabandoned'  => realtimequiz_attempt::ABANDONED,
    ];
    ...
    /**
     * @var array|null of realtimequiz_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = [realtimequiz_attempt::IN_PROGRESS, realtimequiz_attempt::OVERDUE,
            realtimequiz_attempt::FINISHED, realtimequiz_attempt::ABANDONED];
     ...
     * @param stdClass $realtimequiz the settings for the realtimequiz being reported on.
     * @param stdClass $cm the course module objects for the realtimequiz being reported on.
     * @param stdClass $course the course settings for the coures this realtimequiz is in.
     */
    public function __construct($mode, $realtimequiz, $cm, $course) {
        $this->mode   = $mode;
        $this->realtimequiz   = $realtimequiz;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = realtimequiz_report_should_show_grades($realtimequiz, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/realtimequiz/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('realtimequiz_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('realtimequiz_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        ...
        if (!realtimequiz_report_can_filter_only_graded($this->realtimequiz)) {
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(realtimequiz_attempt::FINISHED, $this->states);
    }
}
