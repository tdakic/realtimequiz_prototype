...
/**
 * Base class for realtimequiz report plugins.
 *
 * Doesn't do anything on its own -- it needs to be extended.
 * This class displays realtimequiz reports.  Because it is called from
 * within /mod/realtimequiz/report.php you can assume that the page header
 * and footer are taken care of.
 *
 * This file can refer to itself as report.php to pass variables
 * to itself - all these will also be globally available.  You must
 * pass "id=$cm->id" or q=$realtimequiz->id", and "mode=reportname".
 *
...
abstract class report_base {
     ...
     */
    abstract public function display($realtimequiz, $cm, $course);
    ...
    public function print_header_and_tabs($cm, $course, $realtimequiz, $reportmode = 'overview') {
        ...
        $PAGE->set_title($realtimequiz->name);
        $PAGE->set_heading($course->fullname);
        echo $OUTPUT->header();
        $context = context_module::instance($cm->id);
        if (!$PAGE->has_secondary_navigation()) {
            echo $OUTPUT->heading(format_string($realtimequiz->name, true, ['context' => $context]));
        }
