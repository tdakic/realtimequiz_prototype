...
/**
 * Base class for realtimequiz reports that are basically a table with one row for each attempt.
 *
...
abstract class attempts_report extends report_base {
    ...
    /** @var realtimequiz_settings|null realtimequiz settings object. Set in the init method. */
    protected $realtimequizobj = null;

    /**
     * Can be used in subclasses to cache this information, but it will only get set if you set it.
     * @example an example use in realtimequiz_overview_report.
     *
     * @var bool
     */
     ...
     */
    public function init($mode, $formclass, $realtimequiz, $cm, $course): array {
        $this->mode = $mode;
        $this->realtimequizobj = new realtimequiz_settings($realtimequiz, $cm, $course);
        $this->context = $this->realtimequizobj->get_context();

        [$currentgroup, $studentsjoins, $groupstudentsjoins, $allowedjoins] = $this->get_students_joins(
                $cm, $course);

        $this->qmsubselect = realtimequiz_report_qm_filter_select($realtimequiz);

        $this->form = new $formclass($this->get_base_url(),
                ['realtimequiz' => $realtimequiz, 'currentgroup' => $currentgroup, 'context' => $this->context]);
    ...
    protected function get_students_joins($cm, $course = null) {
        ...
        $groupstudentsjoins = get_enrolled_with_capabilities_join($this->context, '',
                ['mod/realtimequiz:attempt', 'mod/realtimequiz:reviewmyattempts'], $currentgroup);

        return [$currentgroup, $studentsjoins, $groupstudentsjoins, $groupstudentsjoins];
    }

    /**
     * Outputs the things you commonly want at the top of a realtimequiz report.
     *
     ...
     * @param bool $hasquestions whether there are any questions in the realtimequiz.
     * @param bool $hasstudents whether there are any relevant students.
     */
    protected function print_standard_header_and_messages($cm, $course, $realtimequiz,
            $options, $currentgroup, $hasquestions, $hasstudents) {
        global $OUTPUT;

        $this->print_header_and_tabs($cm, $course, $realtimequiz, $this->mode);
        ...
        if ($strattemptnum = realtimequiz_num_attempt_summary($realtimequiz, $cm, true, $currentgroup)) {
            echo '<div class="realtimequizattemptcounts">' . $strattemptnum . '</div>';
        }
    ...
    protected function add_state_column(&$columns, &$headers) {
        global $PAGE;
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'realtimequiz');
        $PAGE->requires->js_call_amd('mod_realtimequiz/reopen_attempt_ui', 'init');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'realtimequiz');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'realtimequiz');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'realtimequiz');
    }
     ...
     * and $headers arrays.
     * @param stdClass $realtimequiz the realtimequiz settings.
     * @param bool $usercanseegrades whether the user is allowed to see grades for this realtimequiz.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($realtimequiz, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'realtimequiz') . '/' .
                    realtimequiz_format_grade($realtimequiz, $realtimequiz->grade);
        }

        if ($includefeedback && realtimequiz_has_feedback($realtimequiz)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'realtimequiz');
        }
    }
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', [], PARAM_INT)) {
                    require_capability('mod/realtimequiz:deleteattempts', $this->context);
                    $this->delete_selected_attempts($realtimequiz, $cm, $attemptids, $allowedjoins);
    ...
    /**
     * Delete the realtimequiz attempts
     * @param stdClass $realtimequiz the realtimequiz settings. Attempts that don't belong to
     * this realtimequiz are not deleted.
     ...
     */
    protected function delete_selected_attempts($realtimequiz, $cm, $attemptids, \core\dml\sql_join $allowedjoins) {
        global $DB;

        foreach ($attemptids as $attemptid) {
            if (empty($allowedjoins->joins)) {
                $sql = "SELECT realtimequiza.*
                          FROM {realtimequiz_attempts} realtimequiza
                          JOIN {user} u ON u.id = realtimequiza.userid
                         WHERE realtimequiza.id = :attemptid";
            } else {
                $sql = "SELECT realtimequiza.*
                          FROM {realtimequiz_attempts} realtimequiza
                          JOIN {user} u ON u.id = realtimequiza.userid
                        {$allowedjoins->joins}
                         WHERE {$allowedjoins->wheres} AND realtimequiza.id = :attemptid";
            }
            $params = $allowedjoins->params + ['attemptid' => $attemptid];
            $attempt = $DB->get_record_sql($sql, $params, IGNORE_MULTIPLE);
            if (!$attempt || $attempt->realtimequiz != $realtimequiz->id || $attempt->preview != 0) {
                // Ensure the attempt exists, belongs to this realtimequiz and belongs to
                // a student included in the report. If not skip.
                continue;
            }

            // Set the course module id before calling realtimequiz_delete_attempt().
