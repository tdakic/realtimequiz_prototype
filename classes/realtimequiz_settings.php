...
/**
 * A class encapsulating the settings for a realtimequiz.
 *
...
class realtimequiz_settings {
    ...
    /** @var stdClass[] of realtimequiz_section rows. */
    protected $sections = null;
    /** @var access_manager the access manager for this realtimequiz. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/realtimequiz:preview. */
     ...
     *
     * @param stdClass $realtimequiz the row from the realtimequiz table.
     * @param stdClass $cm the course_module object for this realtimequiz.
     ...
     */
    public function __construct($realtimequiz, $cm, $course, $getcontext = true) {
        $this->realtimequiz = $realtimequiz;
        $this->cm = $cm;
        $this->realtimequiz->cmid = $this->cm->id;
        $this->course = $course;
     ...
     * @return realtimequiz_settings the new realtimequiz settings object.
     */
    protected static function create_helper(stdClass $realtimequiz, cm_info|stdClass $cm, stdClass $course, ?int $userid): self {
        // Update realtimequiz with override information.
        if ($userid) {
            $realtimequiz = realtimequiz_update_effective_access($realtimequiz, $userid);
        }

        return new realtimequiz_settings($realtimequiz, $cm, $course);
    }

    /**
     * Static function to create a new realtimequiz settings object from a realtimequiz id, for a specific user.
     *
     * @param int $realtimequizid the realtimequiz id.
     * @param int|null $userid the the userid (optional). If passed, relevant overrides are applied.
     * @return realtimequiz_settings the new realtimequiz settings object.
     */
    public static function create(int $realtimequizid, int $userid = null): self {
        $realtimequiz = access_manager::load_realtimequiz_and_settings($realtimequizid);
        ...
        return self::create_helper($realtimequiz, $cm, $course, $userid);
    }

    /**
     * Static function to create a new realtimequiz settings object from a cmid, for a specific user.
     *
    ...
    public static function create_for_cmid(int $cmid, int $userid = null): self {
        [$course, $cm] = get_course_and_cm_from_cmid($cmid, 'realtimequiz');
        $realtimequiz = access_manager::load_realtimequiz_and_settings($cm->instance);

        return self::create_helper($realtimequiz, $cm, $course, $userid);
    }

    /**
     * Create a {@see realtimequiz_attempt} for an attempt at this realtimequiz.
     *
     * @param stdClass $attemptdata row from the realtimequiz_attempts table.
     * @return realtimequiz_attempt the new realtimequiz_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new realtimequiz_attempt($attemptdata, $this->realtimequiz, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================.

    /**
     * Load just basic information about all the questions in this realtimequiz.
     */
    public function preload_questions() {
        $slots = qbank_helper::get_question_structure($this->realtimequiz->id, $this->context);
        $this->questions = [];
    ...
    /**
     * Fully load some or all of the questions for this realtimequiz. You must call
    ...
    public function load_questions($deprecated = null) {
        if ($deprecated !== null) {
            debugging('The argument to realtimequiz::load_questions is no longer supported. ' .
    ...
    /**
     * Get an instance of the {@see \mod_realtimequiz\structure} class for this realtimequiz.
     *
     * @return structure describes the questions in the realtimequiz.
     */
    public function get_structure() {
        return structure::create_for_realtimequiz($this);
    }

    // Simple getters ==========================================================.

    /**
     * Get the id of the course this realtimequiz belongs to.
     *
    ...
    /**
     * Get the course settings object that this realtimequiz belongs to.
     *
    ...
    /**
     * Get this realtimequiz's id (in the realtimequiz table).
     *
     * @return int the realtimequiz id.
     */
    public function get_realtimequizid() {
        return $this->realtimequiz->id;
    }

    /**
     * Get the realtimequiz settings object.
     *
     * @return stdClass the row of the realtimequiz table.
     */
     ...
     *
     * @return stdClass the row of the realtimequiz table.
     * TTT added this to try to fix a bug quick and dirty
     */
    public function get_quiz() {
        return $this->realtimequiz;
    }
    /**
     * Get the realtimequiz name.
     *
     * @return string the name of this realtimequiz.
     */
    public function get_realtimequiz_name() {
        return $this->realtimequiz->name;
    }
    ...
    public function get_navigation_method() {
        return $this->realtimequiz->navmethod;
    }

    /**
     * How many attepts is the user allowed at this realtimequiz?
     *
     * @return int the number of attempts allowed at this realtimequiz (0 = infinite).
     */
    public function get_num_attempts_allowed() {
        return $this->realtimequiz->attempts;
    }

    /**
     * Get the course-module id for this realtimequiz.
     *
    ...
    /**
     * Get the course-module object for this realtimequiz.
     *
     ...
     *
     * @return context_module the module context for this realtimequiz.
     */
    ...
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/realtimequiz:preview', $this->context);
        }
    ...
    public function is_participant($userid) {
        return is_enrolled($this->get_context(), $userid, 'mod/realtimequiz:attempt', $this->show_only_active_users());
    }
    ...
    /**
     * Have any questions been added to this realtimequiz yet?
     *
     * @return bool whether any questions have been added to this realtimequiz.
     */
    ...
    /**
     * Get a particular question in this realtimequiz, by its id.
     *
    ...
    /**
     * Get some of the question in this realtimequiz.
     *
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'realtimequiz', $this->view_url());
            }
            $questions[$id] = $this->questions[$id];
            //TTT removed the line as a quick exception throwing fix
    ...
    /**
     * Get all the sections in this realtimequiz.
     *
     * @return array 0, 1, 2, ... => realtimequiz_sections row from the database.
     */
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new access_manager($this, $timenow,
                    has_capability('mod/realtimequiz:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Return the grade_calculator object for this realtimequiz.
     *
    ...
    /**
     * Wrapper round the has_capability funciton that automatically passes in the realtimequiz context.
     *
     * @param string $capability the name of the capability to check. For example mod/realtimequiz:view.
     ...
     *
     * @return moodle_url the URL of this realtimequiz's view page.
     */
    public function view_url() {
        return new moodle_url('/mod/realtimequiz/view.php', ['id' => $this->cm->id]);
    }

    /**
     * Get the URL of this realtimequiz's edit questions page.
     *
     * @return moodle_url the URL of this realtimequiz's edit page.
     */
    public function edit_url() {
        return new moodle_url('/mod/realtimequiz/edit.php', ['cmid' => $this->cm->id]);
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        ...
        return new moodle_url('/mod/realtimequiz/attempt.php', $params);
    }
    ...
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/realtimequiz/startattempt.php', $params);
    }

    /**
     * Get the URL to review a particular realtimequiz attempt.
     *
     * @param int $attemptid the id of an attempt.
     * @return string the URL of the review of that attempt.
     */
    public function review_url($attemptid) {
        return new moodle_url('/mod/realtimequiz/review.php', ['attempt' => $attemptid, 'cmid' => $this->get_cmid()]);
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/realtimequiz/summary.php', ['attempt' => $attemptid, 'cmid' => $this->get_cmid()]);
    }
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == display_options::DURING ||
            ...
            if ($when == display_options::LATER_WHILE_OPEN && $this->realtimequiz->timeclose &&
                    $this->realtimequiz->reviewattempt & display_options::AFTER_CLOSE) {
                return get_string('noreviewuntil' . $langstrsuffix, 'realtimequiz',
                        userdate($this->realtimequiz->timeclose, $dateformat));
            } else {
                return get_string('noreview' . $langstrsuffix, 'realtimequiz');
            }
        }
    }
     ...
     *
     * @param string $title the name of this particular realtimequiz page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_exception('questionnotloaded', 'realtimequiz', $this->view_url(), $id);
        }
    }

    /**
     * Return all the question types used in this realtimequiz.
     *
     * @param boolean $includepotential if the realtimequiz include random questions,
