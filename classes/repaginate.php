 ...
 *
 * The realtimequiz setting allows users to write realtimequizzes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the realtimequiz being manipulated. */
    private $realtimequizid;
    /** @var array the realtimequiz_slots for that realtimequiz. */
    private $slots;

    /**
     * Constructor.
     * @param int $realtimequizid the id of the realtimequiz being manipulated.
     * @param stdClass[] $slots the realtimequiz_slots for that realtimequiz.
     */
    public function __construct($realtimequizid = 0, $slots = null) {
        global $DB;
        $this->realtimequizid = $realtimequizid;
        if (!$this->realtimequizid) {
            $this->slots = [];
        }
        if (!$slots) {
            $this->slots = $DB->get_records('realtimequiz_slots', ['realtimequizid' => $this->realtimequizid], 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('realtimequiz_slots', $nextslot, true);
     ...
     * Repaginate the rest.
     *
     * @param stdClass[] $realtimequizslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($realtimequizslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$realtimequizslots) {
            return null;
        }
        $newslots = [];
        foreach ($realtimequizslots as $slot) {
            if ($type == self::LINK) {
