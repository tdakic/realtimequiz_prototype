...
/**
 * This class aggregates the access rules that apply to a particular realtimequiz.
 *
 * This provides a convenient API which other parts of the realtimequiz code can use
...
class access_manager {
    /** @var realtimequiz_settings the realtimequiz settings object. */
    protected $realtimequizobj;

    /** @var int the time to be considered as 'now'. */
    protected $timenow;

    /** @var access_rule_base instances of the active rules for this realtimequiz. */
    protected $rules = [];

    /**
     * Create an instance for a particular realtimequiz.
     *
     * @param realtimequiz_settings $realtimequizobj the realtimequiz settings.
     *      The realtimequiz we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/realtimequiz:ignoretimelimits', ...)).
     */
    public function __construct(realtimequiz_settings $realtimequizobj, int $timenow, bool $canignoretimelimits) {
        $this->realtimequizobj = $realtimequizobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($realtimequizobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular realtimequiz.
     *
     * @param realtimequiz_settings $realtimequizobj information about the realtimequiz in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/realtimequiz:ignoretimelimits capability.
     * @return access_rule_base[] rules that apply to this realtimequiz.
     */
    protected function make_rules(realtimequiz_settings $realtimequizobj, int $timenow, bool $canignoretimelimits): array {

        $rules = [];
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($realtimequizobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['realtimequizaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes(): array {
        return core_component::get_plugin_list_with_class('realtimequizaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the realtimequiz table.
     *
     * @param mod_realtimequiz_mod_form $realtimequizform the realtimequiz settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_realtimequiz_mod_form $realtimequizform, MoodleQuickForm $mform): void {

        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($realtimequizform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices(): array {
        $options = ['-' => get_string('none', 'realtimequiz')];
        foreach (self::get_rule_classes() as $rule) {
    ...
    public static function validate_settings_form_fields(array $errors,
            array $data, array $files, mod_realtimequiz_mod_form $realtimequizform): array {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $realtimequizform);
        }
    ...
    /**
     * Save any submitted settings when the realtimequiz settings form is submitted.
     *
     ...
     */
    public static function save_settings(stdClass $realtimequiz): void {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($realtimequiz);
        }
    }

    /**
     * Delete any rule-specific settings when the realtimequiz is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the realtimequiz table.
     *
     ...
     */
    public static function delete_settings(stdClass $realtimequiz): void {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($realtimequiz);
        }
    }
     ...
     */
    protected static function get_load_sql(int $realtimequizid, array $rules, string $basefields): array {
        $allfields = $basefields;
        $alljoins = '{realtimequiz} realtimequiz';
        $allparams = ['realtimequizid' => $realtimequizid];

        foreach ($rules as $rule) {
            [$fields, $joins, $params] = $rule::get_settings_sql($realtimequizid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return ["SELECT $allfields FROM $alljoins WHERE realtimequiz.id = :realtimequizid", $allparams];
    }
     ...
     */
    public static function load_settings(int $realtimequizid): array {
        global $DB;

        $rules = self::get_rule_classes();
        [$sql, $params] = self::get_load_sql($realtimequizid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($realtimequizid);
        }
    ...
    /**
     * Load the realtimequiz settings and any settings required by the access rules.
     ...
     *
     * @param int $realtimequizid the realtimequiz id.
     * @return stdClass mdl_realtimequiz row with extra fields.
     */
    public static function load_realtimequiz_and_settings(int $realtimequizid): stdClass {
        global $DB;

        $rules = self::get_rule_classes();
        [$sql, $params] = self::get_load_sql($realtimequizid, $rules, 'realtimequiz.*');
        $realtimequiz = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($realtimequizid) as $name => $value) {
                $realtimequiz->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this realtimequiz, such
     * as is shown at the top of the realtimequiz view page. Note that not all
    ...
    /**
     * Whether a user should be allowed to start a new attempt at this realtimequiz now.
    ...
    public function get_preflight_check_form(moodle_url $url, ?int $attemptid): preflight_check_form {
        ...
        return new preflight_check_form($url->out_omit_querystring(),
                ['rules' => $this->rules, 'realtimequizobj' => $this->realtimequizobj,
     ...
     *
     * @param stdClass $attempt the data from the relevant realtimequiz_attempts row.
    ...
    /**
     * Is this realtimequiz required to be shown in a popup window?
     *
    ...
    /**
     * Send the user back to the realtimequiz view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param renderer $output the realtimequiz renderer.
    ...
    public function back_to_view_page(renderer $output, string $message = ''): void {
        ...
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup(new moodle_url($this->realtimequizobj->view_url()), $message);
            die();
        } else {
            redirect($this->realtimequizobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the realtimequiz, if that is appropriate.
     *
     * @param stdClass $attempt the attempt object
     * @param mixed $nolongerused not used any more.
     * @param renderer $output realtimequiz renderer instance.
    ...
    public function make_review_link(stdClass $attempt, $nolongerused, renderer $output): string {
        ...
        if (in_array($attempt->state, [realtimequiz_attempt::IN_PROGRESS, realtimequiz_attempt::OVERDUE])) {
            return $output->no_review_message('');
        }

        $when = realtimequiz_attempt_state($this->realtimequizobj->get_realtimequiz(), $attempt);
        $reviewoptions = display_options::make_from_realtimequiz(
                $this->realtimequizobj->get_realtimequiz(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->realtimequizobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->realtimequizobj->review_url($attempt->id),
