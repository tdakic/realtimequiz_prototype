 ...
 * @since      Moodle 3.1
 */
class mod_realtimequiz_external extends external_api {

    /**
     * Describes the parameters for get_realtimequizzes_by_courses.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_realtimequizzes_by_courses_parameters() {
    ...
    /**
     * Returns a list of realtimequizzes in a provided list of courses,
     * if no list is provided all realtimequizzes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of realtimequizzes details
     * @since Moodle 3.1
     */
    public static function get_realtimequizzes_by_courses($courseids = []) {
        global $USER;

        $warnings = [];
        $returnedrealtimequizzes = [];

        $params = [
            'courseids' => $courseids,
        ];
        $params = self::validate_parameters(self::get_realtimequizzes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the realtimequizzes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $realtimequizzes = get_all_instances_in_courses("realtimequiz", $courses);
            foreach ($realtimequizzes as $realtimequiz) {
                $context = context_module::instance($realtimequiz->coursemodule);

                // Update realtimequiz with override information.
                $realtimequiz = realtimequiz_update_effective_access($realtimequiz, $USER->id);

                // Entry to return.
                $realtimequizdetails = helper_for_get_mods_by_courses::standard_coursemodule_element_values(
                        $realtimequiz, 'mod_realtimequiz', 'mod/realtimequiz:view', 'mod/realtimequiz:view');

                if (has_capability('mod/realtimequiz:view', $context)) {
                    $realtimequizdetails['introfiles'] = util::get_area_files($context->id, 'mod_realtimequiz', 'intro', false, false);
                    ...
                    $hasfeedback = realtimequiz_has_feedback($realtimequiz);
                    $realtimequizdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;

                    $timenow = time();
                    $realtimequizobj = realtimequiz_settings::create($realtimequiz->id, $USER->id);
                    $accessmanager = new access_manager($realtimequizobj, $timenow, has_capability('mod/realtimequiz:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the realtimequiz.
                    if (!$accessmanager->prevent_access()) {
                        $realtimequizdetails['hasquestions'] = (int) $realtimequizobj->has_questions();
                        $realtimequizdetails['autosaveperiod'] = get_config('realtimequiz', 'autosaveperiod');
                        ...
                        // Any course module fields that previously existed in realtimequiz.
                        $realtimequizdetails['completionpass'] = $realtimequizobj->get_cm()->completionpassgrade;
                    }
                    ...
                    foreach ($viewablefields as $field) {
                        $realtimequizdetails[$field] = $realtimequiz->{$field};
                    }
                }
                $returnedrealtimequizzes[] = $realtimequizdetails;
            }
        }
        $result = [];
        $result['realtimequizzes'] = $returnedrealtimequizzes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_realtimequizzes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_realtimequizzes_by_courses_returns() {
        return new external_single_structure(
            [
                'realtimequizzes' => new external_multiple_structure(
                    new external_single_structure(array_merge(
                        helper_for_get_mods_by_courses::standard_coursemodule_elements_returns(true),
                        [
                            'timeopen' => new external_value(PARAM_INT, 'The time when this realtimequiz opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this realtimequiz closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for realtimequiz attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a realtimequiz attempt.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their realtimequiz
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    \mod_realtimequiz\question\display_options class. It is formed by ORing
                                                                    together the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their realtimequiz
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their realtimequiz attempts
                            ...
                            'reviewspecificfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            realtimequiz attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewgeneralfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            realtimequiz attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewrightanswer' => new external_value(PARAM_INT, 'Whether users are allowed to review their realtimequiz
                                                                        attempts at various times. A bit field, like
                                                                        reviewattempt.', VALUE_OPTIONAL),
                            'reviewoverallfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their realtimequiz
                                                                            attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'questionsperpage' => new external_value(PARAM_INT, 'How often to insert a page break when editing
                                                                        the realtimequiz, or when shuffling the question order.',
                                                                        VALUE_OPTIONAL),
                            'navmethod' => new external_value(PARAM_ALPHA, 'Any constraints on how the user is allowed to navigate
                                                                around the realtimequiz. Currently recognised values are
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the realtimequiz overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the realtimequiz was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a realtimequiz attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this realtimequiz can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark realtimequiz complete when the student has
                            ...
                            'allowofflineattempts' => new external_value(PARAM_INT, 'Whether to allow the realtimequiz to be attempted
                                                                            offline in the mobile app', VALUE_OPTIONAL),
                            'autosaveperiod' => new external_value(PARAM_INT, 'Auto-save delay', VALUE_OPTIONAL),
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the realtimequiz has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the realtimequiz has questions', VALUE_OPTIONAL),
                        ]
                    ))
                ),
    ...
    /**
     * Utility function for validating a realtimequiz.
     *
     * @param int $realtimequizid realtimequiz instance id
     * @return array array containing the realtimequiz, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_realtimequiz($realtimequizid) {
        global $DB;

        // Request and permission validation.
        $realtimequiz = $DB->get_record('realtimequiz', ['id' => $realtimequizid], '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($realtimequiz, 'realtimequiz');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return [$realtimequiz, $course, $cm, $context];
    }

    /**
     * Describes the parameters for view_realtimequiz.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_realtimequiz_parameters() {
        return new external_function_parameters (
            [
                'realtimequizid' => new external_value(PARAM_INT, 'realtimequiz instance id'),
            ]
        );
    }
     ...
     * @since Moodle 3.1
     */
    public static function view_realtimequiz($realtimequizid) {
        global $DB;

        $params = self::validate_parameters(self::view_realtimequiz_parameters(), ['realtimequizid' => $realtimequizid]);
        $warnings = [];

        list($realtimequiz, $course, $cm, $context) = self::validate_realtimequiz($params['realtimequizid']);
    ...
    /**
     * Describes the view_realtimequiz return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_realtimequiz_returns() {
        return new external_single_structure(
            [
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            [
                ...
                'status' => new external_value(PARAM_ALPHA, 'realtimequiz status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given realtimequiz and user.
     *
     * @param int $realtimequizid realtimequiz instance id
     * @param int $userid user id
     * @param string $status realtimequiz status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     */
    public static function get_user_attempts($realtimequizid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $USER;

        $warnings = [];

        $params = [
            'realtimequizid' => $realtimequizid,
            'userid' => $userid,
            'status' => $status,
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/realtimequiz:viewreports', $context);
        }

        // Update realtimequiz with override information.
        $realtimequiz = realtimequiz_update_effective_access($realtimequiz, $params['userid']);
        $attempts = realtimequiz_get_user_attempts($realtimequiz->id, $user->id, $params['status'], $params['includepreviews']);
        $attemptresponse = [];
        foreach ($attempts as $attempt) {
            $reviewoptions = realtimequiz_get_review_options($realtimequiz, $attempt, $context);
            if (!has_capability('mod/realtimequiz:viewreports', $context) &&
                    ($reviewoptions->marks < question_display_options::MARK_AND_MAX || $attempt->state != realtimequiz_attempt::FINISHED)) {
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            [
                ...
                'realtimequiz' => new external_value(PARAM_INT, 'Foreign key reference to the realtimequiz that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this realtimequiz.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this realtimequiz
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time realtimequiz cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a realtimequiz.
     *
     * @param int $realtimequizid realtimequiz instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($realtimequizid, $userid = 0) {
        global $DB, $USER, $CFG;
        ...
        // This code was mostly copied from mod/realtimequiz/view.php. We need to make the web service logic consistent.
        // Get this user's attempts.
        $attempts = realtimequiz_get_user_attempts($realtimequiz->id, $user->id, 'all');
        $canviewgrade = false;
        if ($attempts) {
            if ($USER->id != $user->id) {
                // No need to check the permission here. We did it at by require_capability('mod/realtimequiz:viewreports', $context).
                $canviewgrade = true;
            } else {
                // Work out which columns we need, taking account what data is available in each attempt.
                [$notused, $alloptions] = realtimequiz_get_combined_reviewoptions($realtimequiz, $attempts);
                $canviewgrade = $alloptions->marks >= question_display_options::MARK_AND_MAX;
            }
        }

        $grade = $canviewgrade ? realtimequiz_get_best_grade($realtimequiz, $user->id) : null;
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            [
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given realtimequiz.'),
                'grade' => new external_value(PARAM_FLOAT, 'The grade (only if the user has a grade).', VALUE_OPTIONAL),
                'gradetopass' => new external_value(PARAM_FLOAT, 'The grade to pass the realtimequiz (only if set).', VALUE_OPTIONAL),
                'warnings' => new external_warnings(),
            ]
        );
    }
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($realtimequizid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = realtimequiz_get_user_attempts($realtimequiz->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a realtimequiz.
     *
     * @param int $realtimequizid realtimequiz instance id
     ...
     * @since Moodle 3.1
     */
    public static function start_attempt($realtimequizid, $preflightdata = [], $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$realtimequizobj->has_questions()) {
            throw new moodle_exception('noquestionsfound', 'realtimequiz', $realtimequizobj->view_url());
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            realtimequiz_validate_new_attempt($realtimequizobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$realtimequizobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_exception(array_shift($errors), 'realtimequiz', $realtimequizobj->view_url());
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == realtimequiz_attempt::OVERDUE) {
                    throw new moodle_exception('stateoverdue', 'realtimequiz', $realtimequizobj->view_url());
                } else {
                    throw new moodle_exception('attemptstillinprogress', 'realtimequiz', $realtimequizobj->view_url());
                }
            }
            $offlineattempt = WS_SERVER ? true : false;
            $attempt = realtimequiz_prepare_and_start_new_attempt($realtimequizobj, $attemptnumber, $lastattempt, $offlineattempt);
        }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = realtimequiz_attempt::create($params['attemptid']);
        ...
        } else if ($failifoverdue && $attemptobj->get_state() == realtimequiz_attempt::OVERDUE) {
            throw new moodle_exception('stateoverdue', 'realtimequiz', $attemptobj->view_url());
        }

        // User submitted data (like the realtimequiz password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_exception(array_shift($errors), 'realtimequiz', $attemptobj->view_url());
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_exception('Invalid page number', 'realtimequiz', $attemptobj->view_url());
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_exception('Out of sequence access', 'realtimequiz', $attemptobj->view_url());
            }
     ...
     *
     * @return external_single_structure the question data. Some fields may not be returned depending on the realtimequiz display settings.
     * @since  Moodle 3.1
    ...
    private static function question_structure() {
        return new external_single_structure(
            [
                ...
                'page' => new external_value(PARAM_INT, 'page of the realtimequiz this question appears on'),
                ...
                'state' => new external_value(PARAM_ALPHA, 'the state where the question is in.
                    It will not be returned if the user cannot see it due to the realtimequiz display correctness settings.',
                    VALUE_OPTIONAL),
            ...
            'The question data. Some fields may not be returned depending on the realtimequiz display settings.'
        );
    }
     ...
     *
     * @param  realtimequiz_attempt  $attemptobj  the realtimequiz attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(realtimequiz_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = [];
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_realtimequiz');
        $contextid = $attemptobj->get_realtimequizobj()->get_context()->id;
    ...
    /**
     * Returns information for the given attempt page for a realtimequiz attempt in progress.
     *
        ...
        return new external_single_structure(
            [
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/realtimequiz:preview capability,
    ...
    /**
     * Returns a summary of a realtimequiz attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the realtimequiz. This function is intended for the realtimequiz auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a realtimequiz and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    public static function process_attempt($attemptid, $data, $finishattempt = false, $timeup = false, $preflightdata = []) {
        ...
        $attemptobj = realtimequiz_attempt::create($params['attemptid']);
        $failifoverdue = !($attemptobj->get_realtimequizobj()->get_realtimequiz()->overduehandling == 'graceperiod');
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_exception('attemptclosed', 'realtimequiz', $attemptobj->view_url());
            } else if (!$displayoptions->attempt) {
                throw new moodle_exception('noreview', 'realtimequiz', $attemptobj->view_url(), null,
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = [
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_realtimequizobj()->get_context()->id,
                'content' => $data['content'],
            ];
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = realtimequiz_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_realtimequiz(), false);

        $feedback = $attemptobj->get_overall_feedback($grade);
        if ($displayoptions->overallfeedback && $feedback) {
            $result['additionaldata'][] = [
                'id' => 'feedback',
                'title' => get_string('feedback', 'realtimequiz'),
                'content' => $feedback,
            ];
        }
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            [
                'grade' => new external_value(PARAM_RAW, 'grade for the realtimequiz (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_realtimequiz_feedback_for_grade_parameters() {
        return new external_function_parameters (
            [
                'realtimequizid' => new external_value(PARAM_INT, 'realtimequiz instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given realtimequiz.
     *
     * @param int $realtimequizid realtimequiz instance id
     ...
     * @since Moodle 3.1
     */
    public static function get_realtimequiz_feedback_for_grade($realtimequizid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_realtimequiz_feedback_for_grade_parameters(), $params);
        $warnings = [];
        ...
        $feedback = realtimequiz_feedback_record_for_grade($params['grade'], $realtimequiz);
        if (!empty($feedback->feedbacktext)) {
            ...
            $feedbackinlinefiles = util::get_area_files($context->id, 'mod_realtimequiz', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_realtimequiz_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_realtimequiz_feedback_for_grade_returns() {
        return new external_single_structure(
            [
    ...
    /**
     * Describes the parameters for get_realtimequiz_access_information.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_realtimequiz_access_information_parameters() {
        return new external_function_parameters (
            [
                'realtimequizid' => new external_value(PARAM_INT, 'realtimequiz instance id')
            ]
    ...
    /**
     * Return access information for a given realtimequiz.
     *
     * @param int $realtimequizid realtimequiz instance id
     * @return array of warnings and the access information
     * @since Moodle 3.1
     */
    public static function get_realtimequiz_access_information($realtimequizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_realtimequiz_access_information_parameters(), $params);

        list($realtimequiz, $course, $cm, $context) = self::validate_realtimequiz($params['realtimequizid']);

        $result = [];
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/realtimequiz:attempt', $context);;
        $result['canmanage'] = has_capability('mod/realtimequiz:manage', $context);;
        $result['canpreview'] = has_capability('mod/realtimequiz:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/realtimequiz:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/realtimequiz:viewreports', $context);;

        // Access manager now.
        $realtimequizobj = realtimequiz_settings::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/realtimequiz:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new access_manager($realtimequizobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_realtimequiz_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_realtimequiz_access_information_returns() {
        return new external_single_structure(
            [
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the realtimequiz or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the realtimequiz settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the realtimequiz or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the realtimequiz reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a realtimequiz.
     *
     * @param int $realtimequizid realtimequiz instance id
     ...
     * @since Moodle 3.1
     */
    public static function get_attempt_access_information($realtimequizid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = realtimequiz_get_user_attempt_unfinished($realtimequiz->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != realtimequiz_attempt::IN_PROGRESS and $unfinishedattempt->state != realtimequiz_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_realtimequiz_required_qtypes.
     *
     * @return external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_realtimequiz_required_qtypes_parameters() {
        return new external_function_parameters (
            [
                'realtimequizid' => new external_value(PARAM_INT, 'realtimequiz instance id')
            ]
    ...
    /**
     * Return the potential question types that would be required for a given realtimequiz.
     ...
     * @since Moodle 3.1
     */
    public static function get_realtimequiz_required_qtypes($realtimequizid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_realtimequiz_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = [];
        $result['questiontypes'] = $realtimequizobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_realtimequiz_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_realtimequiz_required_qtypes_returns() {
        return new external_single_structure(
            [
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the realtimequiz'),
                'warnings' => new external_warnings(),
            ]
        );
    }
