...
/**
 * External api for changing the question version in the realtimequiz.
 *
...
class submit_question_version extends external_api {
    ...
    public static function execute(int $slotid, int $newversion): array {
        ...
        $referencedata = $DB->get_record('question_references',
            ['itemid' => $params['slotid'], 'component' => 'mod_realtimequiz', 'questionarea' => 'slot']);
        $slotdata = $DB->get_record('realtimequiz_slots', ['id' => $slotid]);

        // Capability check.
        [, $cm] = get_course_and_cm_from_instance($slotdata->realtimequizid, 'realtimequiz');
