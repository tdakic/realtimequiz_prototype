 ...
 *
 * The structure of the realtimequiz. That is, which questions it is built up
 * from. This is used on the Edit realtimequiz page (edit.php) and also when
 * starting an attempt at the realtimequiz (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var realtimequiz_settings the realtimequiz this is the structure of. */
    protected $realtimequizobj = null;

    /**
     * @var stdClass[] the questions in this realtimequiz. Contains the row from the questions
     * table, with the data from the realtimequiz_slots table added, and also question_categories.contextid.
     */
    protected $questions = [];

    /** @var stdClass[] realtimequiz_slots.slot => the realtimequiz_slots rows for this realtimequiz, augmented by sectionid. */
    protected $slotsinorder = [];

    /**
     * @var stdClass[] currently a dummy. Holds data that will match the
     * realtimequiz_sections, once it exists.
     */
    protected $sections = [];
    ...
    /**
     * Create an instance of this class representing an empty realtimequiz.
     *
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given realtimequiz.
     *
     * @param realtimequiz_settings $realtimequizobj the realtimequiz.
     * @return structure
     */
    public static function create_for_realtimequiz($realtimequizobj) {
        $structure = self::create();
        $structure->realtimequizobj = $realtimequizobj;
        $structure->populate_structure();
        return $structure;
    }

    /**
     * Whether there are any questions in the realtimequiz.
     *
     * @return bool true if there is at least one question in the realtimequiz.
     */
    ...
    /**
     * Get the number of questions in the realtimequiz.
     *
     * @return int the number of questions in the realtimequiz.
     */
     ...
     * question_category.contextid, and the realtimequiz_slots data for the question in this realtimequiz.
     */
    ...
    public function can_display_number_be_customised(int $slotnumber): bool {
        return $this->is_real_question($slotnumber) && !realtimequiz_has_attempts($this->realtimequizobj->get_realtimequizid());
    }
    ...
    public function make_slot_display_number_in_place_editable(int $slotid, \context $context): \core\output\inplace_editable {
        ...
        $editable = has_capability('mod/realtimequiz:manage', $context);

        // Get the current value.
        $value = $slot->displaynumber ?? $slot->defaultnumber;
        $displayvalue = s($value);

        return new inplace_editable('mod_realtimequiz', 'slotdisplaynumber', $slotid,
                $editable, $displayvalue, $value,
                get_string('edit_slotdisplaynumber_hint', 'mod_realtimequiz'),
                get_string('edit_slotdisplaynumber_label', 'mod_realtimequiz', $displayvalue));
    }
     ...
     * questions cannot be depended upon because of realtimequiz options.
     *
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->realtimequizobj->get_navigation_method() == QUIZ_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), ['random', 'missingtype'])) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->realtimequizobj->get_realtimequiz()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_realtimequiz', $this->realtimequizobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->realtimequizobj->get_realtimequiz()->preferredbehaviour);
    ...
    /**
     * Get the course id that the realtimequiz belongs to.
     *
     * @return int the course.id for the realtimequiz.
     */
    ...
    /**
     * Get the course module id of the realtimequiz.
     *
     * @return int the course_modules.id for the realtimequiz.
     */
     ...
     *
     * @return context_module the context of the realtimequiz that this is the structure of.
     */
    ...
    /**
     * Get id of the realtimequiz.
     *
     * @return int the realtimequiz.id for the realtimequiz.
     */
     ...
     *
     * @return stdClass the realtimequiz settings row from the database.
     */
     ...
     *
     * @return bool whether this realtimequiz can be repaginated.
     */
     ...
     *
     * @return bool whether the realtimequiz can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !realtimequiz_has_attempts($this->realtimequizobj->get_realtimequizid());
        }
        return $this->canbeedited;
    }

    /**
     * This realtimequiz can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = realtimequiz_attempt_summary_link_to_reports($this->get_realtimequiz(),
                    $this->realtimequizobj->get_cm(), $this->realtimequizobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'realtimequiz',
                    new \moodle_url('/mod/realtimequiz/edit.php', ['cmid' => $this->get_cmid()]), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the realtimequiz.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the realtimequiz.
     *
     * @return int the number of questions that should be on each page of the
     * realtimequiz by default.
     */
    public function get_questions_per_page() {
        return $this->realtimequizobj->get_realtimequiz()->questionsperpage;
    }

    /**
     * Get realtimequiz slots.
     *
     * @return stdClass[] the slots in this realtimequiz.
     */
    ...
    /**
     * Is this slot the last one in the realtimequiz?
     *
     * @param int $slotnumber the index of the slot in question.
     * @return bool whether this slot the last one in the realtimequiz.
     */
    public function is_last_slot_in_realtimequiz($slotnumber) {
        end($this->slotsinorder);
        return $slotnumber == key($this->slotsinorder);
    }

    /**
     * Is this the first section in the realtimequiz?
     *
     * @param stdClass $section the realtimequiz_sections row.
     * @return bool whether this is first section in the realtimequiz.
     */
    ...
    /**
     * Is this the last section in the realtimequiz?
     *
     * @param stdClass $section the realtimequiz_sections row.
     * @return bool whether this is first section in the realtimequiz.
     */
    ...
    /**
     * Get the final slot in the realtimequiz.
     *
     * @return stdClass the realtimequiz_slots for the final slot in the realtimequiz.
     */
     ...
     * @return stdClass the requested realtimequiz_slots row.
     */
    ...
    /**
     * Get all the slots in a section of the realtimequiz.
     *
    ...
    /**
     * Get all the sections of the realtimequiz.
     *
     * @return stdClass[] the sections in this realtimequiz.
     */
    ...
    /**
     * Get the overall realtimequiz grade formatted for display.
     *
     * @return string the maximum grade for this realtimequiz.
     */
    public function formatted_realtimequiz_grade() {
        return realtimequiz_format_grade($this->get_realtimequiz(), $this->get_realtimequiz()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return realtimequiz_format_question_grade($this->get_realtimequiz(), $this->slotsinorder[$slotnumber]->maxmark);
    }

    /**
     * Get the number of decimal places for displaying overall realtimequiz grades or marks.
     *
     * @return int the number of decimal places.
     */
    public function get_decimal_places_for_grades() {
        return $this->get_realtimequiz()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return realtimequiz_get_grade_format($this->get_realtimequiz());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (realtimequiz_has_attempts($this->realtimequizobj->get_realtimequizid())) {
            $reviewlink = realtimequiz_attempt_summary_link_to_reports($this->realtimequizobj->get_realtimequiz(),
                    $this->realtimequizobj->get_cm(), $this->realtimequizobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'realtimequiz', $reviewlink);
        }
    ...
    /**
     * Get the date information about the current state of the realtimequiz.
    ...
    public function get_dates_summary() {
        ...
        $dates = [];
        if ($realtimequiz->timeopen > 0) {
            if ($timenow > $realtimequiz->timeopen) {
                $dates[] = get_string('realtimequizopenedon', 'realtimequiz', userdate($realtimequiz->timeopen));
            } else {
                $dates[] = get_string('realtimequizwillopen', 'realtimequiz', userdate($realtimequiz->timeopen));
            }
        }
        if ($realtimequiz->timeclose > 0) {
            if ($timenow > $realtimequiz->timeclose) {
                $dates[] = get_string('realtimequizclosed', 'realtimequiz', userdate($realtimequiz->timeclose));
            } else {
                $dates[] = get_string('realtimequizcloseson', 'realtimequiz', userdate($realtimequiz->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'realtimequiz');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $realtimequiz->timeopen) {
            $currentstatus = get_string('realtimequizisclosedwillopen', 'realtimequiz',
                    userdate($realtimequiz->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($realtimequiz->timeclose && $timenow <= $realtimequiz->timeclose) {
            $currentstatus = get_string('realtimequizisopenwillclose', 'realtimequiz',
                    userdate($realtimequiz->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($realtimequiz->timeclose && $timenow > $realtimequiz->timeclose) {
            $currentstatus = get_string('realtimequizisclosed', 'realtimequiz');
        } else {
            $currentstatus = get_string('realtimequizisopen', 'realtimequiz');
        }
    ...
    /**
     * Set up this class with the structure for a given realtimequiz.
     */
    protected function populate_structure() {
        global $DB;

        $slots = qbank_helper::get_question_structure($this->realtimequizobj->get_realtimequizid(), $this->realtimequizobj->get_context());
        $this->questions = [];
        $this->slotsinorder = [];
        foreach ($slots as $slotdata) {
            ...
            $slot = clone($slotdata);
            $slot->realtimequizid = $this->realtimequizobj->get_realtimequizid();
            $this->slotsinorder[$slot->slot] = $slot;
        }

        // Get realtimequiz sections in ascending order of the firstslot.
        $this->sections = $DB->get_records('realtimequiz_sections', ['realtimequizid' => $this->realtimequizobj->get_realtimequizid()], 'firstslot');
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slotsinorder as $slot) {
            ...
            if ($question->length == 0) {
                ...
                $slot->defaultnumber = get_string('infoshort', 'realtimequiz');
            } else {
    ...
    public function get_version_choices_for_slot(int $slotnumber): array {
        ...
        foreach ($versions as $version) {
            ...
            if ($version->version === $latestversion->version) {
                $version->versionvalue = get_string('questionversionlatest', 'realtimequiz', $version->version);
            } else {
                $version->versionvalue = get_string('questionversion', 'realtimequiz', $version->version);
            }
        ...
        $alwaysuselatest->versionvalue = get_string('alwayslatest', 'realtimequiz');
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if ($followingslotnumber == $movingslotnumber && !$this->is_last_slot_in_realtimequiz($followingslotnumber)) {
        ...
        } else if (!$this->is_last_slot_in_realtimequiz($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_realtimequiz($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('realtimequiz_slots', 'slot', $slotreorder,
                    ['realtimequizid' => $this->get_realtimequizid()]);
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('realtimequiz_slots', 'page', $page,
                    ['id' => $movingslot->id]);
        }

        // Update section fist slots.
        realtimequiz_update_section_firstslots($this->get_realtimequizid(), $headingmovedirection,
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE realtimequizid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {realtimequiz_slots} WHERE realtimequizid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", [$this->get_realtimequizid(), $this->get_realtimequizid()]);

        foreach ($emptypages as $emptypage) {
            $DB->execute("
                    UPDATE {realtimequiz_slots}
                       SET page = page - 1
                     WHERE realtimequizid = ?
                       AND page > ?
                    ", [$this->get_realtimequizid(), $emptypage]);
        }

        $trans->allow_commit();

        // Log slot moved event.
        $event = \mod_realtimequiz\event\slot_moved::create([
    ...
    /**
     * Refresh page numbering of realtimequiz slots.
    ...
    public function refresh_page_numbers($slots = []) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('realtimequiz_slots', ['realtimequizid' => $this->get_realtimequizid()], 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of realtimequiz slots and save to the database.
     *
     * @return stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('realtimequiz_slots', 'page', $slot->page,
                    ['id' => $slot->id]);
        }
    ...
    /**
     * Remove a slot from a realtimequiz.
     *
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('realtimequiz_slots', ['realtimequizid' => $this->get_realtimequizid(), 'slot' => $slotnumber]);
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {realtimequiz_slots} WHERE realtimequizid = ?', [$this->get_realtimequizid()]);

        $trans = $DB->start_delegated_transaction();
        // Delete the reference if it is a question.
        $questionreference = $DB->get_record('question_references',
                ['component' => 'mod_realtimequiz', 'questionarea' => 'slot', 'itemid' => $slot->id]);
        ...
        if ($questionsetreference) {
            $DB->delete_records('question_set_references',
                ['id' => $questionsetreference->id, 'component' => 'mod_realtimequiz', 'questionarea' => 'slot']);
        }
        $DB->delete_records('realtimequiz_slots', ['id' => $slot->id]);
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('realtimequiz_slots', 'slot', $i - 1,
                    ['realtimequizid' => $this->get_realtimequizid(), 'slot' => $i]);
            $this->slotsinorder[$i]->slot = $i - 1;
            $this->slotsinorder[$i - 1] = $this->slotsinorder[$i];
            unset($this->slotsinorder[$i]);
        }

        realtimequiz_update_section_firstslots($this->get_realtimequizid(), -1, $slotnumber);
        ...
        $event = \mod_realtimequiz\event\slot_deleted::create([
            'context' => $this->realtimequizobj->get_context(),
            'objectid' => $slot->id,
            'other' => [
                'realtimequizid' => $this->get_realtimequizid(),
     ...
     *
     * Save changes to the question grades in the realtimequiz_slots table and any
     * corresponding question_attempts.
     *
     * It does not update 'sumgrades' in the realtimequiz table.
     *
     * @param stdClass $slot row from the realtimequiz_slots table.
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('realtimequiz_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new qubaids_for_realtimequiz($slot->realtimequizid),
                $slot->slot, $maxmark);
        $trans->allow_commit();

        // Log slot mark updated event.
        // We use $num + 0 as a trick to remove the useless 0 digits from decimals.
        $event = \mod_realtimequiz\event\slot_mark_updated::create([
            'context' => $this->realtimequizobj->get_context(),
            'objectid' => $slot->id,
            'other' => [
                'realtimequizid' => $this->get_realtimequizid(),
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('realtimequiz_slots', 'requireprevious', $requireprevious, ['id' => $slotid]);

        // Log slot require previous event.
        $event = \mod_realtimequiz\event\slot_requireprevious_updated::create([
            'context' => $this->realtimequizobj->get_context(),
            'objectid' => $slotid,
            'other' => [
                'realtimequizid' => $this->get_realtimequizid(),
    ...
    public function update_slot_display_number(int $slotid, string $displaynumber): void {
        global $DB;
        $DB->set_field('realtimequiz_slots', 'displaynumber', $displaynumber, ['id' => $slotid]);
        $this->populate_structure();

        // Log slot displaynumber event (customised question number).
        $event = \mod_realtimequiz\event\slot_displaynumber_updated::create([
                'context' => $this->realtimequizobj->get_context(),
                'objectid' => $slotid,
                'other' => [
                        'realtimequizid' => $this->get_realtimequizid(),
     ...
     *
     * Save changes to the slot page relationship in the realtimequiz_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $realtimequizslots = $DB->get_records('realtimequiz_slots', ['realtimequizid' => $this->get_realtimequizid()], 'slot');
        $repaginate = new repaginate($this->get_realtimequizid(), $realtimequizslots);
        $repaginate->repaginate_slots($realtimequizslots[$slotid]->slot, $type);
        $slots = $this->refresh_page_numbers_and_update_db();

        if ($type == repaginate::LINK) {
            // Log page break created event.
            $event = \mod_realtimequiz\event\page_break_deleted::create([
                'context' => $this->realtimequizobj->get_context(),
                'objectid' => $slotid,
                'other' => [
                    'realtimequizid' => $this->get_realtimequizid(),
                    'slotnumber' => $realtimequizslots[$slotid]->slot
                ]
            ]);
            $event->trigger();
        } else {
            // Log page deleted created event.
            $event = \mod_realtimequiz\event\page_break_created::create([
                'context' => $this->realtimequizobj->get_context(),
                'objectid' => $slotid,
                'other' => [
                    'realtimequizid' => $this->get_realtimequizid(),
    ...
    public function add_section_heading($pagenumber, $heading = null) {
        ...
        if ($heading !== null) {
            ...
            $section->heading = get_string('newsectionheading', 'realtimequiz');
        }
        $section->realtimequizid = $this->get_realtimequizid();
        $slotsonpage = $DB->get_records('realtimequiz_slots', ['realtimequizid' => $this->get_realtimequizid(), 'page' => $pagenumber], 'slot DESC');
        $firstslot = end($slotsonpage);
        $section->firstslot = $firstslot->slot;
        $section->shufflequestions = 0;
        $sectionid = $DB->insert_record('realtimequiz_sections', $section);

        // Log section break created event.
        $event = \mod_realtimequiz\event\section_break_created::create([
            'context' => $this->realtimequizobj->get_context(),
            'objectid' => $sectionid,
            'other' => [
                'realtimequizid' => $this->get_realtimequizid(),
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('realtimequiz_sections', ['id' => $id], '*', MUST_EXIST);
        $section->heading = $newheading;
        $DB->update_record('realtimequiz_sections', $section);

        // Log section title updated event.
        $firstslot = $DB->get_record('realtimequiz_slots', ['realtimequizid' => $this->get_realtimequizid(), 'slot' => $section->firstslot]);
        $event = \mod_realtimequiz\event\section_title_updated::create([
            'context' => $this->realtimequizobj->get_context(),
            'objectid' => $id,
            'other' => [
                'realtimequizid' => $this->get_realtimequizid(),
    ...
    public function set_section_shuffle($id, $shuffle) {
        ...
        $event = \mod_realtimequiz\event\section_shuffle_updated::create([
            'context' => $this->realtimequizobj->get_context(),
            'objectid' => $id,
            'other' => [
                'realtimequizid' => $this->get_realtimequizid(),
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('realtimequiz_sections', ['id' => $sectionid], '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a realtimequiz.');
        }
        $DB->delete_records('realtimequiz_sections', ['id' => $sectionid]);

        // Log page deleted created event.
        $firstslot = $DB->get_record('realtimequiz_slots', ['realtimequizid' => $this->get_realtimequizid(), 'slot' => $section->firstslot]);
        $event = \mod_realtimequiz\event\section_break_deleted::create([
            'context' => $this->realtimequizobj->get_context(),
            'objectid' => $sectionid,
            'other' => [
                'realtimequizid' => $this->get_realtimequizid(),
    ...
    /**
     * Whether the current user can add random questions to the realtimequiz or not.
    ...
    public function can_add_random_questions() {
        if ($this->canaddrandom === null) {
            $realtimequizcontext = $this->realtimequizobj->get_context();
            $relatedcontexts = new \core_question\local\bank\question_edit_contexts($realtimequizcontext);
