...
/**
 * Render overrides action in the realtimequiz secondary navigation
 *
...
class overrides_actions implements renderable, templatable {
     ...
     *
     * @param \renderer_base $output an instance of the realtimequiz renderer.
     * @return \single_button the button, ready to reander.
     */
    public function create_add_button(\renderer_base $output): \single_button {
        $addoverrideurl = new moodle_url('/mod/realtimequiz/overrideedit.php',
                ['cmid' => $this->cmid, 'action' => 'add' . $this->mode]);

        if ($this->mode === 'group') {
            $label = get_string('addnewgroupoverride', 'realtimequiz');
        } else {
            $label = get_string('addnewuseroverride', 'realtimequiz');
        }
    ...
    public function export_for_template(renderer_base $output): array {
        ...
        $useroverridesurl = new moodle_url('/mod/realtimequiz/overrides.php', ['cmid' => $this->cmid, 'mode' => 'user']);
        $groupoverridesurl = new moodle_url('/mod/realtimequiz/overrides.php', ['cmid' => $this->cmid, 'mode' => 'group']);

        $menu = [
            $useroverridesurl->out(false) => get_string('useroverrides', 'realtimequiz'),
            $groupoverridesurl->out(false) => get_string('groupoverrides', 'realtimequiz')
        ];

        $overridesnav = new url_select($menu, $PAGE->url->out(false), null, 'realtimequizoverrides');
