...
/**
 * Renderer outputting the realtimequiz editing UI.
 *
...
class edit_renderer extends \plugin_renderer_base {
    ...
    protected $togglegroup = 'realtimequiz-questions';

    /**
     * Render the edit page
     *
     * @param \mod_realtimequiz\realtimequiz_settings $realtimequizobj object containing all the realtimequiz settings information.
     * @param structure $structure object containing the structure of the realtimequiz.
     ...
     */
    public function edit_page(\mod_realtimequiz\realtimequiz_settings $realtimequizobj, structure $structure,
        \core_question\local\bank\question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading(get_string('questions', 'realtimequiz'));

        // Information at the top.
        $output .= $this->realtimequiz_state_warnings($structure);

        $output .= html_writer::start_div('mod_realtimequiz-edit-top-controls');
        ...
        $output .= $this->realtimequiz_information($structure);
        $output .= html_writer::end_tag('div');
        $output .= $this->maximum_grade_input($structure, $pageurl);
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_div('d-flex justify-content-between flex-wrap mb-1');
        $output .= html_writer::start_div('mod_realtimequiz-edit-action-buttons btn-group edit-toolbar', ['role' => 'group']);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->selectmultiple_button($structure);
        $output .= html_writer::end_tag('div');

        $output .= html_writer::start_div('d-flex flex-column justify-content-around');
        $output .= $this->total_marks($realtimequizobj->get_realtimequiz());
        $output .= html_writer::end_tag('div');
        $output .= html_writer::end_tag('div');
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->js_call_amd('mod_realtimequiz/realtimequizquestionbank', 'init', [
                $thiscontext->id
            ]);

            $this->page->requires->js_call_amd('mod_realtimequiz/add_random_question', 'init', [
                $thiscontext->id,
                $pagevars['cat'],
    ...
    /**
     * Render any warnings that might be required about the state of the realtimequiz,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the realtimequiz structure.
     * @return string HTML to output.
     */
    public function realtimequiz_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function realtimequiz_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'realtimequiz', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'realtimequizopeningstatus',
                    ['title' => $explanation]);
    ...
    /**
     * Render the form for setting a realtimequiz' overall grade
     *
     * @param structure $structure the realtimequiz structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::start_tag('form', ['method' => 'post', 'action' => 'edit.php',
                'class' => 'realtimequizsavegradesform form-inline']);
        ...
        $output .= html_writer::empty_tag('input', ['type' => 'text', 'id' => 'inputmaxgrade',
                ...
                'value' => $structure->formatted_realtimequiz_grade(),
                'class' => 'form-control']);
        $output .= html_writer::empty_tag('input', ['type' => 'submit', 'class' => 'btn btn-secondary ml-1',
                'name' => 'savechanges', 'value' => get_string('save', 'realtimequiz')]);
     ...
     * @param structure $structure the structure of the realtimequiz being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {
        $header = html_writer::tag('span', get_string('repaginatecommand', 'realtimequiz'), ['class' => 'repaginatecommand']);
        ...
        $buttonoptions = [
            ...
            'value' => get_string('repaginatecommand', 'realtimequiz'),
        ...
        if (!$structure->can_be_repaginated()) {
            ...
            $this->page->requires->js_call_amd('mod_realtimequiz/repaginate', 'init');
        }
    ...
    protected function selectmultiple_button(structure $structure) {
        $buttonoptions = [
            ...
            'value' => get_string('selectmultipleitems', 'realtimequiz'),
        ...
        return html_writer::tag('button', get_string('selectmultipleitems', 'realtimequiz'), $buttonoptions);
    }
    ...
    protected function selectmultiple_controls(structure $structure) {
        ...
        $buttondeleteoptions = [
            ...
            'value' => get_string('deleteselected', 'mod_realtimequiz'),
            'class' => 'btn btn-secondary',
            'data-action' => 'toggle',
        ...
        $output .= html_writer::tag('div',
                        html_writer::tag('button', get_string('deleteselected', 'mod_realtimequiz'), $buttondeleteoptions) .
                        " " .
        ...
        $toolbaroptions = [
            ...
            'role' => 'toolbar',
            'aria-label' => get_string('selectmultipletoolbar', 'realtimequiz'),
        ];
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = [];
        $perpage[0] = get_string('allinone', 'realtimequiz');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'realtimequiz', $select) .
    ...
    /**
     * Render the total marks available for the realtimequiz.
     *
     * @param \stdClass $realtimequiz the realtimequiz settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($realtimequiz) {
        $totalmark = html_writer::span(realtimequiz_format_grade($realtimequiz, $realtimequiz->sumgrades), 'mod_realtimequiz_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'realtimequiz', $totalmark),
                ['class' => 'totalpoints']);
    }
     ...
     *
     * @param structure $structure the structure of the realtimequiz being edited.
     * @param \stdClass $section The realtimequiz_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if ($section->heading) {
            ...
            $sectionheadingtext = get_string('sectionnoname', 'realtimequiz');
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'realtimequiz', $sectionheadingtext),
     ...
     *
     * @param structure $structure object containing the structure of the realtimequiz.
     * @param \stdClass $section data from the realtimequiz_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'realtimequiz');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'realtimequiz'),
    ...
    /**
     * Render an icon to remove a section from the realtimequiz.
     *
     * @param stdClass $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'realtimequiz', format_string($section->heading));
        $url = new \moodle_url('/mod/realtimequiz/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the realtimequiz.
     *
     * This function calls {@link core_course_renderer::realtimequiz_section_question()}
     *
     * @param structure $structure object containing the structure of the realtimequiz.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_realtimequiz($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-realtimequiz-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'realtimequiz'), ['class' => 'add-menu']);
        ...
        // Disable the link if realtimequiz has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(['addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'], 'realtimequiz');
        }

        // Get section, page, slotnumber and maxmark.
        $actions = [];

        // Add a new question to the realtimequiz.
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'realtimequiz', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'realtimequiz');
        }
        ...
        if ($structure->can_add_random_questions()) {
            $returnurl = new \moodle_url('/mod/realtimequiz/edit.php', ['cmid' => $structure->get_cmid(), 'data-addonpage' => $page]);
            $params = ['returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion'];
            $url = new \moodle_url('/mod/realtimequiz/addrandom.php', $params);
            ...
            if ($page) {
                $title = get_string('addrandomquestiontopage', 'realtimequiz', $page);
            } else {
                $title = get_string('addrandomquestionatend', 'realtimequiz');
            }
    ...
    /**
     * Render the form that contains the data for adding a new question to the realtimequiz.
     *
     * @param structure $structure object containing the structure of the realtimequiz.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, int $slot, \moodle_url $pageurl) {
        ...
        if ($structure->get_slot_by_number($slot)->qtype !== 'random') {
            ...
            $this->page->requires->js_call_amd('mod_realtimequiz/question_slot', 'init', [$slotid]);
        }

        // Render the question slot template.
        $output .= $this->render_from_template('mod_realtimequiz/question_slot', $data);
    ...
    public function get_checkbox_render(structure $structure, int $slot) : string {
        ...
        $checkbox = new \core\output\checkbox_toggleall($this->togglegroup, false,
            [
                ...
                'label' => get_string('selectquestionslot', 'realtimequiz', $questionslot),
    ...
    public function get_action_icon(structure $structure, int $slot, \moodle_url $pageurl) : string {
        ...
        $questionicons = '';
        if ($qtype !== 'random') {
            $questionicons .= $this->question_preview_icon($structure->get_realtimequiz(),
    ...
    public function question_number(string $editablenumber, string $uncustomisednumber) {
        if ($editablenumber !== get_string('infoshort', 'realtimequiz')) {
     ...
     * @return string HTML to output.
     */
    public function question_preview_icon($realtimequiz, $questiondata, $label = null, $variant = null, $restartversion = null) {
        ...
        $url = realtimequiz_question_preview_url($realtimequiz, $question, $variant, $restartversion);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'realtimequiz');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'realtimequiz');
    ...
    /**
     * Render an icon to remove a question from the realtimequiz.
     *
     * @param structure $structure object containing the structure of the realtimequiz.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the realtimequiz.
     *
     * @param structure $structure object containing the structure of the realtimequiz.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', ['realtimequizid' => $structure->get_realtimequizid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()]);

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'realtimequiz');
            $image = $this->image_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'realtimequiz');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'realtimequiz', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'realtimequiz'),
                    'moodle', ['title' => '']);
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'realtimequiz', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'realtimequiz'),
                    'moodle', ['title' => '']);
    ...
    /**
     * Renders html to display a name with the link to the question on a realtimequiz edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = realtimequiz_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                ['title' => get_string('editquestion', 'realtimequiz').' '.$title]);
    ...
    public function random_question(structure $structure, $slotnumber, $pageurl) {
        ...
        $editurl = new \moodle_url('/mod/realtimequiz/editrandom.php',
                ['returnurl' => $pageurl->out_as_local_url(), 'slotid' => $slot->id]);

        $temp = clone($question);
        $temp->questiontext = '';
        $temp->name = qbank_helper::describe_random_question($slot);
        $instancename = realtimequiz_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'realtimequiz');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), ['title' => $namestr,
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                        get_string('seequestions', 'realtimequiz'), ['class' => 'mod_realtimequiz_random_qbank_link']);
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                ['title' => get_string('maxmark', 'realtimequiz')]);
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                [
                    ...
                    'title' => get_string('editmaxmark', 'realtimequiz'),
                ]
            )
        );
    ...
    public function render_question_chooser(renderable $chooser) {
        return $this->render_from_template('mod_realtimequiz/question_chooser', $chooser->export_for_template($this));
    }
    ...
    public function question_chooser() {
        $chooser = \mod_realtimequiz\output\question_chooser::get($this->page->course, [], null);
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/realtimequiz/edit_rest.php';
        $config->sectionurl = '/mod/realtimequiz/edit_rest.php';
        $config->pageparams = [];
        ...
        $this->page->requires->yui_module('moodle-mod_realtimequiz-toolboxes',
                'M.mod_realtimequiz.init_resource_toolbox',
                [[
                        'courseid' => $structure->get_courseid(),
                        'realtimequizid' => $structure->get_realtimequizid(),
        ...
        $this->page->requires->strings_for_js(['areyousureremoveselected'], 'realtimequiz');
        $this->page->requires->yui_module('moodle-mod_realtimequiz-toolboxes',
                'M.mod_realtimequiz.init_section_toolbox',
                [[
                        'courseid' => $structure,
                        'realtimequizid' => $structure->get_realtimequizid(),
        ...
        $this->page->requires->yui_module('moodle-mod_realtimequiz-dragdrop', 'M.mod_realtimequiz.init_section_dragdrop',
                [[
                        'courseid' => $structure,
                        'realtimequizid' => $structure->get_realtimequizid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_realtimequiz-dragdrop', 'M.mod_realtimequiz.init_resource_dragdrop',
                [[
                        'courseid' => $structure,
                        'realtimequizid' => $structure->get_realtimequizid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'realtimequiz', '');
     ...
     *
     * @param \mod_realtimequiz\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_realtimequiz\question\bank\custom_view $questionbank, array $pagevars) {
