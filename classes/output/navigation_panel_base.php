...
abstract class navigation_panel_base {
     ...
     * Constructor.
     *
     * @param realtimequiz_attempt $attemptobj construct the panel for this attempt.
     * @param question_display_options $options display options in force.
     * @param int $page which page of the realtimequiz attempt is being shown, -1 if all.
     * @param bool $showall whether all pages are being shown at once.
     */
    public function __construct(realtimequiz_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the realtimequiz navigation block.
     *
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            ...
            if (!is_null($heading)) {
                $sections = $this->attemptobj->get_realtimequizobj()->get_sections();
            ...
            $button->id          = 'realtimequiznavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'realtimequiz');
            }
            $buttons[] = $button;
        }
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'realtimequiz');
        } else {
            return get_string('viewed', 'realtimequiz');
        }
    }
     ...
     *
     * @param renderer $output the realtimequiz renderer to use.
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_realtimequiz()->showuserpicture == QUIZ_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', ['id' => $this->attemptobj->get_userid()]);
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_realtimequiz()->showuserpicture == QUIZ_SHOWIMAGE_LARGE) {
