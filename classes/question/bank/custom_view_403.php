...
/**
 * Defines the custom question bank view used on the Edit  page.
 *
...
namespace mod_realtimequiz\question\bank;
...
use mod_realtimequiz\question\bank\filter\custom_category_condition;
use qbank_managecategories\category_condition;

require_once($CFG->dirroot . '/mod/realtimequiz/locallib.php');
/**
 * Subclass to customise the view of the question bank for the realtimequiz editing screen.
 *
...
class custom_view extends \core_question\local\bank\view {
    ...
    /** @var bool $realtimequizhasattempts whether the realtimequiz this is used by has been attemptd. */
    protected $realtimequizhasattempts = false;

    /** @var \stdClass $realtimequiz the realtimequiz settings. */
    protected $realtimequiz = false;

    /**
     * @var string $component the component the api is used from.
     */
    public $component = 'mod_realtimequiz';
     ...
     * @param \stdClass $realtimequiz realtimequiz settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $params, $extraparams) {
        ...
        [$this->realtimequiz, ] = get_module_from_cmid($cm->id);
        $this->set_realtimequiz_has_attempts(realtimequiz_has_attempts($this->realtimequiz->id));
    ...
    protected function get_question_bank_plugins(): array {
        ...
        $customviewcolumns = [
            'mod_realtimequiz\question\bank\add_action_column' . column_base::ID_SEPARATOR  . 'add_action_column',
            ...
            'mod_realtimequiz\question\bank\question_name_text_column' . column_base::ID_SEPARATOR . 'question_name_text_column',
            'mod_realtimequiz\question\bank\preview_action_column'  . column_base::ID_SEPARATOR  . 'preview_action_column',
        ];
    ...
    protected function heading_column(): string {
        return 'mod_realtimequiz\\question\\bank\\question_name_text_column';
    }

    protected function default_sort(): array {
        // Using the extended class for realtimequiz specific sort.
        return [
            'qbank_viewquestiontype__question_type_column' => SORT_ASC,
            'mod_realtimequiz__question__bank__question_name_text_column' => SORT_ASC,
        ];
    }

    /**
     * Let the question bank display know whether the realtimequiz has been attempted,
     * hence whether some bits of UI, like the add this question to the realtimequiz icon,
     * should be displayed.
     *
     * @param bool $realtimequizhasattempts whether the realtimequiz has attempts.
     */
    private function set_realtimequiz_has_attempts($realtimequizhasattempts): void {
        $this->realtimequizhasattempts = $realtimequizhasattempts;
        if ($realtimequizhasattempts && isset($this->visiblecolumns['addtorealtimequizaction'])) {
            unset($this->visiblecolumns['addtorealtimequizaction']);
        }
    }

    /**
     * URL of add to realtimequiz.
     *
     * @param $questionid
     * @return \moodle_url
     */
    public function add_to_realtimequiz_url($questionid) {
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        $params['cmid'] = $this->cm->id;
        return new \moodle_url('/mod/realtimequiz/edit.php', $params);
    }
    ...
    protected function display_bottom_controls(\context $catcontext): void {
        ...
        $cmoptions->hasattempts = !empty($this->realtimequizhasattempts);

        $canuseall = has_capability('moodle/question:useall', $catcontext);

        echo \html_writer::start_tag('div', ['class' => 'pt-2']);
        if ($canuseall) {
            // Add selected questions to the realtimequiz.
            $params = [
                'type' => 'submit',
                'name' => 'add',
                'class' => 'btn btn-primary',
                'value' => get_string('addselectedquestionstorealtimequiz', 'realtimequiz'),
                'data-action' => 'toggle',
        ...
        foreach ($this->plugins as $componentname => $plugin) {
            if (\core\plugininfo\qbank::is_plugin_enabled($componentname)) {
                ...
                if ($componentname === 'qbank_managecategories') {
                    $pluginentrypointobject = new realtimequiz_managecategories_feature();
                }
    ...
    /**
     * Return the realtimequiz settings for the realtimequiz this question bank is displayed in.
     *
