...
class qbank_helper {
    ...
    /**
     * Get the information about which questions should be used to create a realtimequiz attempt.
     *
     ...
     *
     * @param int $realtimequizid the id of the realtimequiz to load the data for.
     * @param \context_module $realtimequizcontext the context of this realtimequiz.
     * @param int|null $slotid optional, if passed only load the data for this one slot (if it is in this realtimequiz).
     * @return array indexed by slot, with information about the content of each slot.
     */
    public static function get_question_structure(int $realtimequizid, \context_module $realtimequizcontext,
            int $slotid = null): array {
        global $DB;

        $params = [
            'draft' => question_version_status::QUESTION_STATUS_DRAFT,
            'realtimequizcontextid' => $realtimequizcontext->id,
            'realtimequizcontextid2' => $realtimequizcontext->id,
            'realtimequizcontextid3' => $realtimequizcontext->id,
            'realtimequizid' => $realtimequizid,
            'realtimequizid2' => $realtimequizid,
        ];
        $slotidtest = '';
        $slotidtest2 = '';
        if ($slotid !== null) {
        ...
        $slotdata = $DB->get_records_sql("
             ...
             -- case where a particular question has been added to the realtimequiz.
             LEFT JOIN {question_references} qr ON qr.usingcontextid = :realtimequizcontextid AND qr.component = 'mod_realtimequiz'
             ...
             LEFT JOIN (
                   SELECT lv.questionbankentryid,
                     ...
                     FROM {realtimequiz_slots} lslot
                     JOIN {question_references} lqr ON lqr.usingcontextid = :realtimequizcontextid2 AND lqr.component = 'mod_realtimequiz'
                    ...
                    WHERE lslot.realtimequizid = :realtimequizid2
                          $slotidtest2
             ...
             LEFT JOIN {question_set_references} qsr ON qsr.usingcontextid = :realtimequizcontextid3 AND qsr.component = 'mod_realtimequiz'
                                        AND qsr.questionarea = 'slot' AND qsr.itemid = slot.id

                 WHERE slot.realtimequizid = :realtimequizid
                       $slotidtest
        ...
        foreach ($slotdata as $slot) {
            ...
            if ($slot->filtercondition) {
                ...
                $slot->qtype = 'random';
                $slot->name = get_string('random', 'realtimequiz');
                $slot->length = 1;
                ...
                $slot->name = get_string('missingquestion', 'realtimequiz');
                $slot->questiontext = ' ';
     ...
     * @param int $realtimequizid the id of the realtimequiz to load the data for.
     * @param \context_module $realtimequizcontext the context of this realtimequiz.
     * @param int $slotid optional, if passed only load the data for this one slot (if it is in this realtimequiz).
     ...
     */
    public static function choose_question_for_redo(int $realtimequizid, \context_module $realtimequizcontext,
            int $slotid, qubaid_condition $qubaids): int {
        $slotdata = self::get_question_structure($realtimequizid, $realtimequizcontext, $slotid);
