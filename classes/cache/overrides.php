...
/**
 * Cache data source for the realtimequiz overrides.
 *
...
namespace mod_realtimequiz\cache;

use cache_definition;

/**
 * Class realtimequiz_overrides
 *
 * @package   mod_realtimequiz
 * @copyright 2021 Shamim Rezaie <shamim@moodle.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class overrides implements \cache_data_source {
    ...
    public function load_for_cache($key) {
        ...
        [$realtimequizid, $ug, $ugid] = explode('_', $key);
        $realtimequizid = (int) $realtimequizid;

        switch ($ug) {
            case 'u':
                $userid = (int) $ugid;
                $override = $DB->get_record(
                    'realtimequiz_overrides',
                    ['realtimequiz' => $realtimequizid, 'userid' => $userid],
            ...
            case 'g':
                ...
                $override = $DB->get_record(
                    'realtimequiz_overrides',
                    ['realtimequiz' => $realtimequizid, 'groupid' => $groupid],
                    'timeopen, timeclose, timelimit, attempts, password'
                );
                break;
            default:
