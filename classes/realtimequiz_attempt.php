...
use Exception;
use html_writer;
use mod_realtimequiz\output\links_to_other_attempts;
...
/**
 * This class represents one user's attempt at a particular realtimequiz.
 *
 * @package   mod_realtimequiz
 * @copyright 2008 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class realtimequiz_attempt {
    ...
    /** @var int maximum number of slots in the realtimequiz for the review page to default to show all. */
    ...
    /** @var stdClass the realtimequiz_attempts row. */
    protected $attempt;
    ...
    /** @var array of realtimequiz_sections rows, with a ->lastslot field added. */
    protected $sections;
     ...
     *
     * @param stdClass $attempt the row of the realtimequiz_attempts table.
     * @param stdClass $realtimequiz the realtimequiz object for this attempt and user.
     * @param stdClass|cm_info $cm the course_module object for this realtimequiz.
     ...
     */
    public function __construct($attempt, $realtimequiz, $cm, $course, $loadquestions = true) {
        $this->attempt = $attempt;
        $this->realtimequizobj = new realtimequiz_settings($realtimequiz, $cm, $course);
     ...
     *
     * @param array $conditions passed to $DB->get_record('realtimequiz_attempts', $conditions).
     * @return realtimequiz_attempt the desired instance of this class.
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('realtimequiz_attempts', $conditions, '*', MUST_EXIST);
        $realtimequiz = access_manager::load_realtimequiz_and_settings($attempt->realtimequiz);
        $course = get_course($realtimequiz->course);
        $cm = get_coursemodule_from_instance('realtimequiz', $realtimequiz->id, $course->id, false, MUST_EXIST);
        ...
        return new realtimequiz_attempt($attempt, $realtimequiz, $cm, $course);
    }

    /**
     * Static function to create a new realtimequiz_attempt object given an attemptid.
     *
     * @param int $attemptid the attempt id.
     * @return realtimequiz_attempt the new realtimequiz_attempt object
     */
    ...
    /**
     * Static function to create a new realtimequiz_attempt object given a usage id.
     *
    ...
    /**
     * Get a human-readable name for one of the realtimequiz attempt states.
     *
     * @param string $state one of the state constants like IN_PROGRESS.
     * @return string the human-readable state name.
     */
    public static function state_name($state) {
        return realtimequiz_attempt_state_name($state);
    }
    ...
    public function load_questions() {
        ...
        if (isset($this->quba)) {
            throw new coding_exception('This realtimequiz attempt has already had the questions loaded.');
        }

        $this->quba = question_engine::load_questions_usage_by_activity($this->attempt->uniqueid);
        $this->slots = $DB->get_records('realtimequiz_slots',
                ['realtimequizid' => $this->get_realtimequizid()], 'slot', 'slot, id, requireprevious, displaynumber');
        $this->sections = array_values($DB->get_records('realtimequiz_sections',
                ['realtimequizid' => $this->get_realtimequizid()], 'firstslot'));
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'realtimequiz');
                }
    ...
    /**
     * Get the raw realtimequiz settings object.
     *
     * @return stdClass
     */
    public function get_realtimequiz() {
        return $this->realtimequizobj->get_realtimequiz();
    }

    /**
     * Get the {@see seb_realtimequiz_settings} object for this realtimequiz.
     *
     * @return realtimequiz_settings
     */
    public function get_realtimequizobj() {
        return $this->realtimequizobj;
    }

    /**
     * Git the id of the course this realtimequiz belongs to.
     *
     * @return int the course id.
     */
    public function get_courseid() {
        return $this->realtimequizobj->get_courseid();
    }
    ...
    public function get_course() {
        return $this->realtimequizobj->get_course();
    }

    /**
     * Get the realtimequiz id.
     *
     * @return int the realtimequiz id.
     */
    public function get_realtimequizid() {
        return $this->realtimequizobj->get_realtimequizid();
    }

    /**
     * Get the name of this realtimequiz.
     *
     * @return string Quiz name, directly from the database (format_string must be called before output).
     */
    public function get_realtimequiz_name() {
        return $this->realtimequizobj->get_realtimequiz_name();
    }

    /**
     * Get the realtimequiz navigation method.
     *
     * @return int QUIZ_NAVMETHOD_FREE or QUIZ_NAVMETHOD_SEQ.
     */
    public function get_navigation_method() {
        return $this->realtimequizobj->get_navigation_method();
    }

    /**
     * Get the course_module for this realtimequiz.
     *
     * @return stdClass|cm_info the course_module object.
     */
    public function get_cm() {
        return $this->realtimequizobj->get_cm();
    }
    ...
    public function get_cmid() {
        return $this->realtimequizobj->get_cmid();
    }

    /**
     * Get the realtimequiz context.
     *
     * @return context_module the context of the realtimequiz this attempt belongs to.
     */
    public function get_context(): context_module {
        return $this->realtimequizobj->get_context();
    }

    /**
     * Is the current user is someone who previews the realtimequiz, rather than attempting it?
     *
    ...
    /**
     * Get the number of attempts the user is allowed at this realtimequiz.
     *
     * @return int the number of attempts allowed at this realtimequiz (0 = infinite).
     */
    public function get_num_attempts_allowed() {
        return $this->realtimequizobj->get_num_attempts_allowed();
    }

    /**
     * Get the number of realtimequizzes in the realtimequiz attempt.
     *
    ...
    /**
     * Get the access_manager for this realtimequiz attempt.
     *
     * @param int $timenow the current time as a unix timestamp.
     * @return access_manager and instance of the access_manager class
     *      for this realtimequiz at this time.
     */
    public function get_access_manager($timenow) {
        return $this->realtimequizobj->get_access_manager($timenow);
    }
    ...
    /**
     * Get the question-usage id corresponding to this realtimequiz attempt.
     *
    ...
    /**
     * Get the raw realtimequiz attempt object.
     *
     * @return stdClass the row from the realtimequiz_attempts table.
     */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/realtimequiz:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the realtimequiz in a non-trivial way?
     *
     ...
     * To see how this works, try setting a realtimequiz to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the realtimequiz
     ...
     *
     * @param question_display_options $options the display options for this realtimequiz attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return realtimequiz_feedback_for_grade($grade, $this->get_realtimequiz(),
                $this->realtimequizobj->get_context());
    }

    /**
     * Wrapper round the has_capability function that automatically passes in the realtimequiz context.
     *
    ...
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->realtimequizobj->has_capability($capability, $userid, $doanything);
    }

    /**
     * Wrapper round the require_capability function that automatically passes in the realtimequiz context.
     *
     * @param string $capability the name of the capability to check. For example mod/forum:view.
     * @param int|null $userid A user id. If null checks the permissions of the current user.
     * @param bool $doanything If false, ignore effect of admin role assignment.
    ...
    public function require_capability($capability, $userid = null, $doanything = true) {
        $this->realtimequizobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/realtimequiz:attempt';
        } else {
            $capability = 'mod/realtimequiz:reviewmyattempts';
        }

        // These next tests are in a slightly funny order. The point is that the
        // common and most performance-critical case is students attempting a realtimequiz,
        // so we want to check that permission first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the realtimequiz as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/realtimequiz:viewreports') ||
                $this->has_capability('mod/realtimequiz:preview')) {
    ...
    /**
     * Get where we are time-wise in relation to this attempt and the realtimequiz settings.
     *
    ...
    public function get_attempt_state() {
        return realtimequiz_attempt_state($this->get_realtimequiz(), $this->attempt);
    }

    /**
     * Wrapper that the correct display_options for this realtimequiz at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = realtimequiz_get_review_options($this->get_realtimequiz(),
                        $this->attempt, $this->realtimequizobj->get_context());
            ...
            $options = display_options::make_from_realtimequiz($this->get_realtimequiz(),
                    display_options::DURING);

            $options->flags = realtimequiz_get_flag_option($this->attempt, $this->realtimequizobj->get_context());
            return $options;
        }
    }
    ...
    // TTT added
    public function get_display_options_RT($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = realtimequiz_get_review_options($this->get_realtimequiz(),
                        $this->attempt, $this->realtimequizobj->get_context());
    ...
    /**
     * Is a particular page the last one in the realtimequiz?
     *
     * @param int $page a page number
     * @return bool true if that is the last page of the realtimequiz.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the realtimequiz, or for the
     * whole realtimequiz.
     *
    ...
    public function get_question_usage() {
        if (!(PHPUNIT_TEST || defined('BEHAT_TEST'))) {
            throw new coding_exception('get_question_usage is only for use in unit tests. ' .
                    'For other operations, use the realtimequiz_attempt api, or extend it properly.');
        }
        return $this->quba;
    }
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_realtimequiz()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the realtimequiz where this question appears.
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the realtimequiz this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the realtimequiz.
     */
    public function get_question_mark($slot) {
        return realtimequiz_format_question_grade($this->get_realtimequiz(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = [];
        if ($this->realtimequizobj->get_realtimequiz()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->realtimequizobj->get_realtimequiz()->timelimit;
        }
        if ($this->realtimequizobj->get_realtimequiz()->timeclose) {
            $deadlines[] = $this->realtimequizobj->get_realtimequiz()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->realtimequizobj->get_realtimequiz()->graceperiod;
    ...
    /**
     * Get the URL of this realtimequiz's view.php page.
     *
     * @return moodle_url realtimequiz view url.
     */
    public function view_url() {
        return $this->realtimequizobj->view_url();
    }
     ...
     * @return moodle_url the URL of this realtimequiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->realtimequizobj->start_attempt_url($page);
    }
    ...
    public function attempt_page_title(int $page) : string {
        if ($this->get_num_pages() > 1) {
            $a = new stdClass();
            $a->name = $this->get_realtimequiz_name();
            $a->currentpage = $page + 1;
            $a->totalpages = $this->get_num_pages();
            $title = get_string('attempttitlepaged', 'realtimequiz', $a);
        } else {
            $title = get_string('attempttitle', 'realtimequiz', $this->get_realtimequiz_name());
        }
    ...
    public function summary_page_title() : string {
        return get_string('attemptsummarytitle', 'realtimequiz', $this->get_realtimequiz_name());
    }

    /**
     * Get the URL of the summary page of this attempt.
     *
     * @return moodle_url the URL of this realtimequiz's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/realtimequiz/summary.php', ['attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()]);
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/realtimequiz/processattempt.php');
    }

    //TTT added
    public function save_attempt_page($timestamp, $processsubmitted, $timefinish = null, $studentisonline = false) {
        ...
        //        !has_capability('mod/realtimequiz:emailnotifyattemptgraded', $this->get_realtimequizobj()->get_context(),
        //                $this->get_userid())) {
        //    $this->attempt->gradednotificationsenttime = $this->attempt->timefinish;
      //  }

        $DB->update_record('realtimequiz_attempts', $this->attempt);

        //if (!$this->is_preview()) {
        //    $this->recompute_final_grade();

            // Trigger event.
            //$this->fire_state_transition_event('\mod_realtimequiz\event\attempt_submitted', $timestamp, $studentisonline);
    ...
    public function review_page_title(int $page, bool $showall = false) : string {
        if (!$showall && $this->get_num_pages() > 1) {
            ...
            $a->currentpage = $page + 1;
            $a->totalpages = $this->get_num_pages();
            $title = get_string('attemptreviewtitlepaged', 'realtimequiz', $a);
        } else {
            $title = get_string('attemptreviewtitle', 'realtimequiz', $this->get_realtimequiz_name());
        }
    ...
    public function cannot_review_message($short = false) {
        return $this->realtimequizobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'realtimequiz'));
        } else {
            return '';
        }
    }
     ...
     * @param renderer $renderer the realtimequiz renderer.
    ...
    public function render_question($slot, $reviewing, renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_realtimequiz-blocked_question_warning');
        }
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'realtimequiz', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'realtimequiz');
        ...
        $placeholderqa->start($this->get_realtimequiz()->preferredbehaviour, 1);
     ...
     *
     * @param int $slot the slot number of a question in this realtimequiz attempt.
     ...
     *
     * @param int $slot the slot of a question in this realtimequiz attempt.
     * @param bool $reviewing is the being printed on an attempt or a review page.
     ...
     *
     * @param renderer $output the realtimequiz renderer to use to output things.
     * @param string $panelclass The type of panel, navigation_panel_attempt::class or navigation_panel_review::class
     * @param int $page the current page number.
     * @param bool $showall whether we are showing the whole realtimequiz on one page. (Used by review.php.)
    ...
    public function get_navigation_panel(renderer $output,
        ...
        $bc->attributes['id'] = 'mod_realtimequiz_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->title = get_string('realtimequiznavigation', 'realtimequiz');
        $bc->content = $output->navigation_panel($panel);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this realtimequiz.
     *
    ...
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = realtimequiz_get_user_attempts($this->get_realtimequiz()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->realtimequizobj->get_realtimequiz()->graceperiod;
        ...
        // Otherwise, we were in realtimequiz_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->realtimequizobj->get_realtimequiz()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_realtimequiz\question\qubaids_for_users_attempts(
                $this->get_realtimequizid(), $this->get_userid(), 'all', true);

        $transaction = $DB->start_delegated_transaction();

        // Add the question to the usage. It is important we do this before we choose a variant.
        $newquestionid = qbank_helper::choose_question_for_redo($this->get_realtimequizid(),
                    $this->get_realtimequizobj()->get_context(), $this->slots[$slot]->id, $qubaids);
        $newquestion = question_bank::load_question($newquestionid, $this->get_realtimequiz()->shuffleanswers);
     ...
     * Submit the attempt.
     *
     * The separate $timefinish argument should be used when the realtimequiz attempt
     ...
     * @param bool $studentisonline is the student currently interacting with Moodle?
     */
    public function process_finish_T($timestamp, $timefinish = null, $studentisonline = false) {
        global $DB;
...
*/
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('realtimequiz_attempts', 'timecheckstate', $time, ['id' => $this->attempt->id]);
        }
    }

    /**
     * Needs to be called after this attempt's grade is changed, to update the overall realtimequiz grade.
     */
    protected function recompute_final_grade(): void {
        $this->realtimequizobj->get_grade_calculator()->recompute_final_grade($this->get_userid());
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_realtimequiz\event\attempt_becameoverdue', $timestamp, $studentisonline);

        $transaction->allow_commit();

        realtimequiz_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_realtimequiz\event\attempt_abandoned', $timestamp, $studentisonline);
    ...
    public function process_reopen_abandoned($timestamp) {
        ...
        $this->fire_state_transition_event('\mod_realtimequiz\event\attempt_reopened', $timestamp, false);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp, $studentisonline) {
        global $USER;
        $realtimequizrecord = $this->get_realtimequiz();
        $params = [
            'context' => $this->get_realtimequizobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => [
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'realtimequizid' => $realtimequizrecord->id,
                'studentisonline' => $studentisonline
            ]
        ];
        $event = $eventclass::create($params);
        $event->add_record_snapshot('realtimequiz', $this->get_realtimequiz());
        $event->add_record_snapshot('realtimequiz_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the realtimequiz.
     * Used by {@see attempt_url()} and {@see review_url()}.
     *
     * @param string $script e.g. 'attempt' or 'review'. Used in the URL like /mod/realtimequiz/$script.php.
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/realtimequiz/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a realtimequiz.
     *
    ...
    /*    echo "<br /> In function finishattempt";
        if ($finishattempt)
        {
          echo "True <br />";
        }
        ...
        $graceperiodmin = get_config('realtimequiz', 'graceperiodmin');
        ...
        if ($timeclose !== false) {
            if ($timenow > $timeclose - QUIZ_MIN_TIME_TO_CONTINUE) {
                ...
                // to show the student another page of the realtimequiz. Just finish now.
                $timeup = true;
        ...
        if ($timeup) {
            if ($this->get_realtimequiz()->overduehandling === 'graceperiod') {
                if ($timenow > $timeclose + $this->get_realtimequiz()->graceperiod + $graceperiodmin) {
        ...
        /*echo "<br /> In function becomingabandoned";
...
*/
        ...
        if (!$finishattempt) {
            ...
            //$this->quba->process_all_actions($timestamp);
            //$this->quba->process_all_actions($timenow, array(1));
            $this->quba->process_all_actions($timenow, $slots_on_this_page);
            // should likely be only for questions on the page
            //$this->quba->finish_all_questions($timestamp);
            //$this->quba->finish_question(1,$timenow);
        ...
        // Update the realtimequiz attempt record.
        try {
            if ($becomingabandoned) {
                $this->process_abandon($timenow, true);
            } else {
                if (!$toolate || $this->get_realtimequiz()->overduehandling === 'graceperiod') {
    ...
    public function process_attempt_T($timenow, $finishattempt, $timeup, $thispage) {
        global $DB;
...
*/
      ...
      //  }
        ...
        $finishattempt = false;
            ...
            //the following causes out of sequence exception
            //$this->process_finish($timenow, !$toolate, $finishtime, true);
            $this->attempt->state = self::IN_PROGRESS;
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('realtimequiz_attempts', 'currentpage', $page, ['id' => $this->get_attemptid()]);
            return true;
        }
        return false;
    }
    ...
    public function fire_attempt_viewed_event() {
        $params = [
            ...
            'other' => [
                'realtimequizid' => $this->get_realtimequizid(),
                'page' => $this->get_currentpage()
            ]
        ];
