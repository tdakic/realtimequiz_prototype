...
/**
 * This file defines the setting form for the realtimequiz grading report.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class realtimequiz_grading_settings_form extends moodleform {
    ...
    /** @var stdClass $context the realtimequiz context. */
    protected $context;

    /**
     * realtimequiz_grading_settings_form constructor.
     *
    ...
    public function __construct(array $hidden, stdClass $counts, bool $shownames, bool $showcustomfields, stdClass $context) {
        ...
        parent::__construct($CFG->wwwroot . '/mod/realtimequiz/report.php');
    }

    protected function definition() {
        $mform = $this->_form;

        $mform->addElement('header', 'options', get_string('options', 'realtimequiz_grading'));
        ...
        foreach (['needsgrading', 'manuallygraded', 'autograded', 'all'] as $type) {
            ...
            $gradeoptions[$type] = get_string('gradeattempts' . $type, 'realtimequiz_grading',
                    $this->counts->$type);
        }
        $mform->addElement('select', 'grade', get_string('attemptstograde', 'realtimequiz_grading'),
                $gradeoptions);

        $mform->addElement('text', 'pagesize', get_string('questionsperpage', 'realtimequiz_grading'),
                ['size' => 3]);
        $mform->addRule('pagesize', null, 'positiveint', null, 'client');
        $mform->setType('pagesize', PARAM_INT);

        $orderoptions = [
            'random' => get_string('random', 'realtimequiz_grading'),
        ...
        $mform->addElement('select', 'order', get_string('orderattemptsby', 'realtimequiz_grading'),
                $orderoptions);
        ...
        $mform->addElement('submit', 'submitbutton', get_string('changeoptions', 'realtimequiz_grading'));
    }
}
