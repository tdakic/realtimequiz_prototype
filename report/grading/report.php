...
require_once($CFG->dirroot . '/mod/realtimequiz/report/grading/gradingsettings_form.php');
 ...
 */
class realtimequiz_grading_report extends report_base {
    ...
    /** @var array from realtimequiz_report_get_significant_questions. */
    protected $questions;
    ...
    /** @var stdClass the realtimequiz settings. */
    protected $realtimequiz;

    /** @var context the realtimequiz context. */
    protected $context;

    /** @var realtimequiz_grading_renderer Renderer of Quiz Grading. */
    protected $renderer;
    ...
    public function display($realtimequiz, $cm, $course) {
        ...
        $pagesize = optional_param('pagesize',
                get_user_preferences('realtimequiz_grading_pagesize', self::DEFAULT_PAGE_SIZE),
                PARAM_INT);
        $page = optional_param('page', 0, PARAM_INT);
        $order = optional_param('order',
                get_user_preferences('realtimequiz_grading_order', self::DEFAULT_ORDER),
                PARAM_ALPHAEXT);
        ...
        require_capability('mod/realtimequiz:grade', $this->context);
        $shownames = has_capability('realtimequiz/grading:viewstudentnames', $this->context);
        // Whether the current user can see custom user fields.
        $showcustomfields = has_capability('realtimequiz/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this realtimequiz.
        $this->questions = realtimequiz_report_get_significant_questions($realtimequiz);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'realtimequiz_grading');
        }
        ...
        if ($this->currentgroup == self::NO_GROUPS_ALLOWED) {
            ...
            $this->userssql = get_enrolled_sql($this->context,
                    ['mod/realtimequiz:reviewmyattempts', 'mod/realtimequiz:attempt'], $this->currentgroup);
        }

        $hasquestions = realtimequiz_has_questions($this->realtimequiz->id);
        if (!$hasquestions) {
            $this->print_header_and_tabs($cm, $course, $realtimequiz, 'grading');
            echo $this->renderer->render_realtimequiz_no_question_notification($realtimequiz, $cm, $this->context);
            return true;
        }
        ...
        if (!$counts || $counts->$grade == 0) {
            redirect($this->list_questions_url(), get_string('alldoneredirecting', 'realtimequiz_grading'));
        }
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "realtimequiza.realtimequiz = :mangrrealtimequizid AND
                realtimequiza.preview = 0 AND
                realtimequiza.state = :statefinished";
        $params = ['mangrrealtimequizid' => $this->cm->instance, 'statefinished' => realtimequiz_attempt::FINISHED];
        ...
        if ($currentgroup) {
            ...
            if ($enrolleduserscount < 1) {
                $where .= ' AND realtimequiza.userid = 0';
            } else {
                $usersjoin = "JOIN ({$userssql[0]}) AS enr ON realtimequiza.userid = enr.id";
                $params += $userssql[1];
            }
        }

        return new qubaid_join("{realtimequiz_attempts} realtimequiza $usersjoin ", 'realtimequiza.uniqueid', $where, $params);
    }

    /**
     * Load the realtimequiz_attempts rows corresponding to a list of question_usage ids.
     *
     * @param int[] $qubaids the question_usage ids of the realtimequiz_attempts to load.
     * @return array realtimequiz attempts, with added user name fields.
     */
    protected function load_attempts_by_usage_ids($qubaids) {
        global $DB;

        list($asql, $params) = $DB->get_in_or_equal($qubaids);
        $params[] = realtimequiz_attempt::FINISHED;
        $params[] = $this->realtimequiz->id;

        $fields = 'realtimequiza.*, ';
        $userfieldsapi = \core_user\fields::for_identity($this->context)->with_name();
        ...
        $attemptsbyid = $DB->get_records_sql("
                ...
                {$userfieldssql->joins}
                WHERE realtimequiza.uniqueid $asql AND realtimequiza.state = ? AND realtimequiza.realtimequiz = ?",
                $params);
    ...
    protected function display_index($includeauto) {
        ...
        $this->print_header_and_tabs($this->cm, $this->course, $this->realtimequiz, 'grading');
        ...
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'realtimequiz_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'realtimequiz_grading');
        }
        echo $this->renderer->render_display_index_heading($linktext, $this->list_questions_url(!$includeauto));
        $data = [];
        $header = [];

        $header[] = get_string('qno', 'realtimequiz_grading');
        $header[] = get_string('qtypeveryshort', 'question');
        $header[] = get_string('questionname', 'realtimequiz_grading');
        $header[] = get_string('tograde', 'realtimequiz_grading');
        $header[] = get_string('alreadygraded', 'realtimequiz_grading');
        if ($includeauto) {
            $header[] = get_string('automaticallygraded', 'realtimequiz_grading');
        }
        $header[] = get_string('total', 'realtimequiz_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new realtimequiz_grading_settings_form($hidden, $counts, $shownames, $showcustomfields, $this->context);
        ...
        if ($mform->is_submitted()) {
            if ($mform->is_validated()) {
                ...
                set_user_preference('realtimequiz_grading_pagesize', $pagesize);
                set_user_preference('realtimequiz_grading_order', $order);
                ...
                $pagesize = get_user_preferences('realtimequiz_grading_pagesize', self::DEFAULT_PAGE_SIZE);
            }
        }
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = realtimequiz_get_review_options($this->realtimequiz, $attempt, $this->context);
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby === 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON realtimequiza.userid = u.id {$userfieldssql->joins}";
     ...
     */
    public function print_header_and_tabs($cm, $course, $realtimequiz, $reportmode = 'overview') {
        global $PAGE;
        $this->renderer = $PAGE->get_renderer('realtimequiz_grading');
        parent::print_header_and_tabs($cm, $course, $realtimequiz, $reportmode);
    }

    /**
     * Get question heading.
     *
     * @param stdClass $attempt An instance of realtimequiz_attempt.
    ...
    protected function get_question_heading(stdClass $attempt, bool $shownames, bool $showcustomfields): string {
        ...
        if ($shownames && $showcustomfields) {
            return get_string('gradingattemptwithcustomfields', 'realtimequiz_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'realtimequiz_grading', $a);
