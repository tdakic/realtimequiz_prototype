 ...
 */
class realtimequiz_responses_settings_form extends attempts_report_options_form {

    protected function other_preference_fields(MoodleQuickForm $mform) {
        $mform->addGroup([
            $mform->createElement('advcheckbox', 'qtext', '',
                get_string('questiontext', 'realtimequiz_responses')),
            $mform->createElement('advcheckbox', 'resp', '',
                get_string('response', 'realtimequiz_responses')),
            $mform->createElement('advcheckbox', 'right', '',
                get_string('rightanswer', 'realtimequiz_responses')),
        ], 'coloptions', get_string('showthe', 'realtimequiz_responses'), [' '], false);
    ...
    public function validation($data, $files) {
        ...
        if ($data['attempts'] != attempts_report::ENROLLED_WITHOUT && !(
            ...
            $errors['coloptions'] = get_string('reportmustselectstate', 'realtimequiz');
        }
