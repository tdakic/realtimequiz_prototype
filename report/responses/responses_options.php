...
use mod_realtimequiz\local\reports\attempts_report_options;

/**
 * Class to store the options for a {@link realtimequiz_responses_report}.
 *
 * @copyright 2012 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class realtimequiz_responses_options extends attempts_report_options {
    ...
    protected function get_url_params() {
        ...
        if (realtimequiz_allows_multiple_tries($this->realtimequiz)) {
    ...
    public function setup_from_user_preferences() {
        ...
        $this->showqtext     = get_user_preferences('realtimequiz_report_responses_qtext', $this->showqtext);
        $this->showresponses = get_user_preferences('realtimequiz_report_responses_resp',  $this->showresponses);
        $this->showright     = get_user_preferences('realtimequiz_report_responses_right', $this->showright);
        if (realtimequiz_allows_multiple_tries($this->realtimequiz)) {
            $this->whichtries    = get_user_preferences('realtimequiz_report_responses_which_tries', $this->whichtries);
        }
    }
    ...
    public function update_user_preferences() {
        ...
        set_user_preference('realtimequiz_report_responses_qtext', $this->showqtext);
        set_user_preference('realtimequiz_report_responses_resp',  $this->showresponses);
        set_user_preference('realtimequiz_report_responses_right', $this->showright);
        if (realtimequiz_allows_multiple_tries($this->realtimequiz)) {
            set_user_preference('realtimequiz_report_responses_which_tries', $this->whichtries);
        }
    }
    ...
    public function resolve_dependencies() {
        ...
        $this->checkboxcolumn = has_capability('mod/realtimequiz:deleteattempts', context_module::instance($this->cm->id))
