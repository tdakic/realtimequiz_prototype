...
namespace realtimequiz_responses;
 ...
 */
class responses_from_steps_walkthrough_test extends \mod_realtimequiz\attempt_walkthrough_from_csv_test {
     ...
     *
     * @param array $realtimequizsettings settings to override default settings for realtimequiz created by generator. Taken from realtimequizzes.csv.
    ...
    public function test_walkthrough_from_csv($realtimequizsettings, $csvdata) {
        ...
        $realtimequizattemptids = $this->walkthrough_attempts($csvdata['steps']);
        ...
        foreach ($csvdata['responses'] as $responsesfromcsv) {
            ...
            if (!isset($realtimequizattemptids[$responses['realtimequizattempt']])) {
                throw new \coding_exception("There is no realtimequizattempt {$responses['realtimequizattempt']}!");
            }
            $this->assert_response_test($realtimequizattemptids[$responses['realtimequizattempt']], $responses);
        }
    }

    protected function assert_response_test($realtimequizattemptid, $responses) {
        $realtimequizattempt = realtimequiz_attempt::create($realtimequizattemptid);
        ...
        foreach ($responses['slot'] as $slot => $tests) {
            ...
            $qa = $realtimequizattempt->get_question_attempt($slot);
            ...
            if (null === $step) {
                throw new \coding_exception("There is no step no {$responses['submittedstepno']} ".
                                           "for slot $slot in realtimequizattempt {$responses['realtimequizattempt']}!");
            }
            foreach (['responsesummary', 'fraction', 'state'] as $column) {
                if (isset($tests[$column]) && $tests[$column] != '') {
                    ...
                    $failuremessage = "Error in  realtimequizattempt {$responses['realtimequizattempt']} in $column, slot $slot, ".
