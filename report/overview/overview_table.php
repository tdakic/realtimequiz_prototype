...
use mod_realtimequiz\local\reports\attempts_report_table;
use mod_realtimequiz\realtimequiz_attempt;

/**
 * This is a table subclass for displaying the realtimequiz grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class realtimequiz_overview_table extends attempts_report_table {
     ...
     * @param realtimequiz_overview_options $options
     * @param \core\dml\sql_join $groupstudentsjoins
     * @param \core\dml\sql_join $studentsjoins
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($realtimequiz, $context, $qmsubselect,
            realtimequiz_overview_options $options, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $studentsjoins, $questions, $reporturl) {
        parent::__construct('mod-realtimequiz-report-overview-report', $realtimequiz , $context,
    ...
    /**
     * Calculate the average overall and question scores for a set of attempts at the realtimequiz.
     *
    ...
    public function compute_average_row($label, \core\dml\sql_join $usersjoins) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(realtimequizaouter.sumgrades) AS grade, COUNT(realtimequizaouter.sumgrades) AS numaveraged
                  FROM {realtimequiz_attempts} realtimequizaouter
                  JOIN (
                       SELECT DISTINCT realtimequiza.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON realtimequizaouter.id = relevant_attempt_ids.id
                ", $params);
        $record->grade = realtimequiz_rescale_grade($record->grade, $this->realtimequiz, false);
        ...
        $averagerow = [
            ...
            'feedbacktext' => strip_tags(realtimequiz_report_feedback_for_grade(
                                         $record->grade, $this->realtimequiz->id, $this->context))
        ];

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join("{realtimequiz_attempts} realtimequizaouter
                  JOIN (
                       SELECT DISTINCT realtimequiza.id
                         FROM $from
                        WHERE $where
                       ) relevant_attempt_ids ON realtimequizaouter.id = relevant_attempt_ids.id",
                    'realtimequizaouter.uniqueid', '1 = 1', $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = realtimequiz_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->realtimequiz, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = realtimequiz_format_question_grade($this->realtimequiz, $record->grade);
        } else {
            $average = realtimequiz_format_grade($this->realtimequiz, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/realtimequiz:regrade', $this->context)) {
            $regradebuttonparams = [
                ...
                'name'  => 'regrade',
                'value' => get_string('regradeselected', 'realtimequiz_overview'),
                'data-action' => 'toggle',
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = realtimequiz_rescale_grade($attempt->sumgrades, $this->realtimequiz);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = realtimequiz_rescale_grade($newsumgrade, $this->realtimequiz);
            $oldsumgrade = realtimequiz_rescale_grade($oldsumgrade, $this->realtimequiz);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/realtimequiz/review.php',
                ['attempt' => $attempt->attempt]), $grade,
                ['title' => get_string('reviewattempt', 'realtimequiz')]);
    }

    /**
     * @param string $colname the name of the column.
     * @param stdClass $attempt the row of data - see the SQL in display() in
     * mod/realtimequiz/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = realtimequiz_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->realtimequiz, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = realtimequiz_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->realtimequiz, 'question');
            $oldgrade = realtimequiz_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'realtimequiz_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'realtimequiz_overview');
        }
    }
    ...
    protected function update_sql_after_count($fields, $from, $where, $params) {
        $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {rt_quiz_overview_regrades} qqr
                                 WHERE qqr.questionusageid = realtimequiza.uniqueid
                          ), -1) AS regraded";
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/realtimequiz:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('rt_quiz_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return realtimequiz_report_index_by_keys($regradedqs, ['questionusageid', 'slot']);
    }
}
