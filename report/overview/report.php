...
/**
 * This file defines the realtimequiz overview report class.
 *
...
require_once($CFG->dirroot . '/mod/realtimequiz/report/overview/overview_options.php');
require_once($CFG->dirroot . '/mod/realtimequiz/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/realtimequiz/report/overview/overview_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class realtimequiz_overview_report extends attempts_report {
    ...
    public function display($realtimequiz, $cm, $course) {
        ...
        $table = new realtimequiz_overview_table($realtimequiz, $this->context, $this->qmsubselect,
                $options, $groupstudentsjoins, $studentsjoins, $questions, $options->get_url());
        $filename = realtimequiz_report_download_filename(get_string('overviewfilename', 'realtimequiz_overview'),
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            list($bands, $bandwidth) = self::get_bands_count_and_width($realtimequiz);
            $labels = self::get_bands_labels($bands, $bandwidth, $realtimequiz);
            ...
            if ($currentgroup && $this->hasgroupstudents) {
                $sql = "SELECT qg.id
                        ...
                        {$groupstudentsjoins->joins}
                          WHERE qg.realtimequiz = $realtimequiz->id AND {$groupstudentsjoins->wheres}";
                if ($DB->record_exists_sql($sql, $groupstudentsjoins->params)) {
                    $data = realtimequiz_report_grade_bands($bandwidth, $bands, $realtimequiz->id, $groupstudentsjoins);
            ...
            if ($DB->record_exists('realtimequiz_grades', ['realtimequiz' => $realtimequiz->id])) {
...
*/
     ...
     */
    protected function process_actions($realtimequiz, $cm, $currentgroup, \core\dml\sql_join $groupstudentsjoins,
            \core\dml\sql_join $allowedjoins, $redirecturl) {
        parent::process_actions($realtimequiz, $cm, $currentgroup, $groupstudentsjoins, $allowedjoins, $redirecturl);
        ...
        if (empty($currentgroup) || $this->hasgroupstudents) {
            if (optional_param('regrade', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', [], PARAM_INT)) {
                    $this->start_regrade($realtimequiz, $cm);
                    $this->regrade_attempts($realtimequiz, false, $groupstudentsjoins, $attemptids);
        ...
        if (optional_param('regradeall', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($realtimequiz, $cm);
            $this->regrade_attempts($realtimequiz, false, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldry', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($realtimequiz, $cm);
            $this->regrade_attempts($realtimequiz, true, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);

        } else if (optional_param('regradealldrydo', 0, PARAM_BOOL) && confirm_sesskey()) {
            $this->start_regrade($realtimequiz, $cm);
            $this->regrade_attempts_needing_it($realtimequiz, $groupstudentsjoins);
            $this->finish_regrade($redirecturl);
        }
    }
     ...
     * @param stdClass $cm the cm object for the realtimequiz.
     */
    protected function start_regrade($realtimequiz, $cm) {
        require_capability('mod/realtimequiz:regrade', $this->context);
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT;
        \core\notification::success(get_string('regradecomplete', 'realtimequiz_overview'));
    ...
    /**
     * Regrade a particular realtimequiz attempt. Either for real ($dryrun = false), or
     * as a pretend regrade to see which fractions would change. The outcome is
     * stored in the rt_quiz_overview_regrades table.
     *
     ...
     *
     * @param stdClass $attempt the realtimequiz attempt to regrade.
    ...
    public function regrade_attempt($attempt, $dryrun = false, $slots = null): array {
        global $DB;
        // Need more time for a realtimequiz with many questions.
        ...
        $messages = [];
        $finished = $attempt->state == realtimequiz_attempt::FINISHED;
        foreach ($slots as $slot) {
            ...
            if (abs($qqr->oldfraction - $qqr->newfraction) > 1e-7) {
                ...
                $DB->insert_record('rt_quiz_overview_regrades', $qqr, false);
            }
        }
        ...
        if (!$dryrun) {
            ...
            $params = [
              ...
              'context' => $this->context,
              'other' => [
                'realtimequizid' => $attempt->realtimequiz
              ]
            ];
    ...
    protected function get_new_question_for_regrade(stdClass $attempt,
        ...
        if ($this->structureforregrade === null) {
            ...
            $this->structureforregrade = qbank_helper::get_question_structure(
                    $attempt->realtimequiz, $this->context);
        }
    ...
    /**
     * Regrade attempts for this realtimequiz, exactly which attempts are regraded is
     ...
     */
    protected function regrade_attempts($realtimequiz, $dryrun = false,
            core\dml\sql_join $groupstudentsjoins = null, $attemptids = []) {
        global $DB;
        $this->unlock_session();

        $userfieldsapi = \core_user\fields::for_name();
        $sql = "SELECT realtimequiza.*, " . $userfieldsapi->get_sql('u', false, '', '', false)->selects . "
                  FROM {realtimequiz_attempts} realtimequiza
                  JOIN {user} u ON u.id = realtimequiza.userid";
        $where = "realtimequiz = :qid AND preview = 0";
        $params = ['qid' => $realtimequiz->id];
        ...
        if ($attemptids) {
            ...
            $where .= " AND realtimequiza.id $attemptidcondition";
        ...
        $this->regrade_batch_of_attempts($realtimequiz, $attempts, $dryrun, $groupstudentsjoins);
    }

    /**
     * Regrade those questions in those attempts that are marked as needing regrading
     * in the rt_quiz_overview_regrades table.
     * @param stdClass $realtimequiz the realtimequiz settings.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     * for these users.
     */
    protected function regrade_attempts_needing_it($realtimequiz, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        $this->unlock_session();

        $join = '{rt_quiz_overview_regrades} qqr ON qqr.questionusageid = realtimequiza.uniqueid';
        $where = "realtimequiza.realtimequiz = :qid AND realtimequiza.preview = 0 AND qqr.regraded = 0";
        $params = ['qid' => $realtimequiz->id];
        ...
        $toregrade = $DB->get_recordset_sql("
                SELECT realtimequiza.uniqueid, qqr.slot
                  FROM {realtimequiz_attempts} realtimequiza
                  JOIN $join
                 WHERE $where", $params);
        ...
        $attempts = $DB->get_records_sql("
                SELECT realtimequiza.*, " . $userfieldsapi->get_sql('u', false, '', '', false)->selects . "
                  FROM {realtimequiz_attempts} realtimequiza
                  JOIN {user} u ON u.id = realtimequiza.userid
                 WHERE realtimequiza.uniqueid $uniqueidcondition
                ", $params);
        ...
        $this->regrade_batch_of_attempts($realtimequiz, $attempts, false, $groupstudentsjoins);
    }
     ...
     * Each object in the attempts array must be a row from the realtimequiz_attempts
     ...
     *
     * @param stdClass $realtimequiz the realtimequiz settings.
     * @param array $attempts of data from the realtimequiz_attempts table, with extra data as above.
     * @param bool $dryrun if true, do a pretend regrade, otherwise do it for real.
     * @param \core\dml\sql_join $groupstudentsjoins empty for all attempts, otherwise regrade attempts
     */
    protected function regrade_batch_of_attempts($realtimequiz, array $attempts,
            bool $dryrun, \core\dml\sql_join $groupstudentsjoins) {
        global $OUTPUT;
        $this->clear_regrade_table($realtimequiz, $groupstudentsjoins);

        $progressbar = new progress_bar('realtimequiz_overview_regrade', 500, true);
        $a = [
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofywithdetails', 'realtimequiz_overview', $a));
            $messages = $this->regrade_attempt($attempt, $dryrun, $attempt->regradeonlyslots);
            if ($messages) {
                $items = [];
                foreach ($messages as $slot => $message) {
                    $items[] = get_string('regradingattemptissue', 'realtimequiz_overview',
                            ['slot' => $slot, 'reason' => $message]);
                }
                echo $OUTPUT->notification(
                        html_writer::tag('p', get_string('regradingattemptxofyproblem', 'realtimequiz_overview', $a)) .
                        html_writer::alist($items), \core\output\notification::NOTIFY_WARNING);
            }
        }
        $progressbar->update($a['done'], $a['count'],
                get_string('regradedsuccessfullyxofy', 'realtimequiz_overview', $a));

        if (!$dryrun) {
            $this->update_overall_grades($realtimequiz);
        }
    }
     ...
     */
    protected function count_question_attempts_needing_regrade($realtimequiz, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        if ($this->hasgroupstudents) {
            $userjoin = "JOIN {user} u ON u.id = realtimequiza.userid
                    {$groupstudentsjoins->joins}";
            $usertest = "{$groupstudentsjoins->wheres} AND u.id = realtimequiza.userid AND ";
            $params = $groupstudentsjoins->params;
        }

        $params['crealtimequiz'] = $realtimequiz->id;
        $sql = "SELECT COUNT(DISTINCT realtimequiza.id)
                  FROM {realtimequiz_attempts} realtimequiza
                  JOIN {rt_quiz_overview_regrades} qqr ON realtimequiza.uniqueid = qqr.questionusageid
                $userjoin
                 WHERE
                      $usertest
                      realtimequiza.realtimequiz = :crealtimequiz AND
    ...
    protected function has_regraded_questions($from, $where, $params) {
        ...
        return $DB->record_exists_sql("
                SELECT 1
                  FROM {$from}
                  JOIN {rt_quiz_overview_regrades} qor ON qor.questionusageid = realtimequiza.uniqueid
                 WHERE {$where}", $params);
    }
     ...
     * to these users is cleared.
     */
    protected function clear_regrade_table($realtimequiz, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;
        ...
        $select = "questionusageid IN (
                    SELECT uniqueid
                      FROM {realtimequiz_attempts} realtimequiza";
        $where = "WHERE realtimequiza.realtimequiz = :qid";
        $params = ['qid' => $realtimequiz->id];
        if ($this->hasgroupstudents && !empty($groupstudentsjoins->joins)) {
            $select .= "\nJOIN {user} u ON u.id = realtimequiza.userid
                    {$groupstudentsjoins->joins}";
        ...
        $DB->delete_records_select('rt_quiz_overview_regrades', $select, $params);
    }
     ...
     * @param stdClass $realtimequiz the realtimequiz settings.
     */
    protected function update_overall_grades($realtimequiz) {
        $gradecalculator = $this->realtimequizobj->get_grade_calculator();
    ...
    /**
     * Get the bands configuration for the realtimequiz.
     *
     * This returns the configuration for having between 11 and 20 bars in
     * a chart based on the maximum grade to be given on a realtimequiz. The width of
     ...
     */
    public static function get_bands_count_and_width($realtimequiz) {
        $bands = $realtimequiz->grade;
        ...
        return [$bands, $realtimequiz->grade / $bands];
    }
     ...
     */
    public static function get_bands_labels($bands, $bandwidth, $realtimequiz) {
        $bandlabels = [];
        for ($i = 1; $i <= $bands; $i++) {
            $bandlabels[] = realtimequiz_format_grade($realtimequiz, ($i - 1) * $bandwidth) . ' - ' . realtimequiz_format_grade($realtimequiz, $i * $bandwidth);
        }
        return $bandlabels;
    }
