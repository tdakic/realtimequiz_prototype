@mod @mod_realtimequiz @realtimequiz @realtimequiz_overview @javascript
Feature: Regrading realtimequiz attempts using the Grades report
    In order to be able to correct mistakes I made setting up my realtimequiz
    As a teacher
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | realtimequiz     | Quiz for testing regrading | C1     | realtimequiz1    |
    And the following "questions" exist:
      | questioncategory | qtype       | name |
      | Test questions   | truefalse   | TF   |
      | Test questions   | shortanswer | SA   |
    And realtimequiz "Quiz for testing regrading" contains the following questions:
  ...
  Scenario: Regrade all attempts
    Given I am on the "Quiz for testing regrading" "realtimequiz activity" page logged in as teacher
    And I navigate to "Results" in current page administration
    When I press "Regrade all"

    # Note, the order is not defined, so we can only check part of the message.
    # Also, nothing has changed in the realtimequiz, so the regrade won't alter any scores,
  ...
  Scenario: Dry-run a full regrade, then regrade the attempts that will need it.
    Given I am on the "Quiz for testing regrading" "mod_realtimequiz > edit" page logged in as teacher
  ...
  Scenario: Regrade all attempts works against realtimequiz selected question version
    Given I am on the "Quiz for testing regrading" "realtimequiz activity" page logged in as teacher
    And I navigate to "Results" in current page administration
    ...
    And I should see "Overall number of students achieving grade ranges"
    And "Student One" row "Regrade" column of "attempts" table should not contain "Needed"
    And I am on the "Quiz for testing regrading" "mod_realtimequiz > question bank" page
    And I choose "Edit question" action for "TF" in the question bank
    And I set the field "Correct answer" to "False"
    And I press "id_submitbutton"
    And I am on the "Quiz for testing regrading" "mod_realtimequiz > edit" page
  ...
  Scenario: Regrade all attempts works against realtimequiz selected latest question version
    Given I am on the "Quiz for testing regrading" "realtimequiz activity" page logged in as teacher
    And I navigate to "Results" in current page administration
    And I click on "mod-realtimequiz-report-overview-report-selectall-attempts" "checkbox"
  ...
  Scenario: Regrade attempts should always regrade against latest random question version
    ...
    And I am on the "Quiz for testing regrading" "realtimequiz activity" page logged in as student3
    And I click on "Attempt realtimequiz" "button"
    ...
    And I am on the "Quiz for testing regrading" "mod_realtimequiz > question bank" page logged in as teacher
    And I choose "Edit question" action for "TF" in the question bank
    And I set the field "Correct answer" to "False"
    And I press "id_submitbutton"
    And I navigate to "Results" in current page administration
