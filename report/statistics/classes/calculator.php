...
/**
 * Class to calculate and also manage caching of realtimequiz statistics.
 *
...
class calculator {
    ...
    /**
     * Compute the realtimequiz statistics.
     *
     ...
     * @return calculated $realtimequizstats The statistics for overall attempt scores.
     */
    public function calculate($realtimequizid, $whichattempts, \core\dml\sql_join $groupstudentsjoins, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $realtimequizstats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($realtimequizid, $groupstudentsjoins);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $realtimequizstats->{$propertyname} = $value;
        }

        $s = $realtimequizstats->s();
        if ($s != 0) {

            // Recalculate sql again this time possibly including test for first attempt.
            list($fromqa, $whereqa, $qaparams) =
                realtimequiz_statistics_attempts_sql($realtimequizid, $groupstudentsjoins, $whichattempts);

            $realtimequizstats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($realtimequizstats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $realtimequizstats->standarddeviation = sqrt($powers->power2 / ($s - 1));
                ...
                if ($s > 2) {
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $realtimequizstats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $realtimequizstats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $realtimequizstats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $realtimequizstats->errorratio = 100 * sqrt(1 - ($realtimequizstats->cic / 100));
                            $realtimequizstats->standarderror = $realtimequizstats->errorratio *
                                $realtimequizstats->standarddeviation / 100;
                        }
                    }
            ...
            $realtimequizstats->cache(realtimequiz_statistics_qubaids_condition($realtimequizid, $groupstudentsjoins, $whichattempts));
        }
        $this->progress->end_progress();
        return $realtimequizstats;
    }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record('realtimequiz_statistics', ['hashcode' => $qubaids->get_hash_code(),
    ...
    public function get_last_calculated_time($qubaids) {
        global $DB;
        $lastcalculatedtime = $DB->get_field('realtimequiz_statistics', 'COALESCE(MAX(timemodified), 0)',
    ...
    /**
     * Given a particular realtimequiz grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'realtimequiz_statistics');
    }

    /**
     * Given a particular realtimequiz grading method return a string id for use as a field name prefix in mdl_realtimequiz_statistics or to
     ...
     */
    protected function attempt_counts_and_averages($realtimequizid, \core\dml\sql_join $groupstudentsjoins) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(realtimequiz_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = realtimequiz_statistics_attempts_sql($realtimequizid, $groupstudentsjoins, $which);
    ...
    protected function median($s, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT realtimequiza.id, realtimequiza.sumgrades
                  FROM $fromqa
                 WHERE $whereqa
              ORDER BY sumgrades";
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((realtimequiza.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((realtimequiza.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((realtimequiza.sumgrades - $mean), 4)) AS power4
                  FROM $fromqa
                 WHERE $whereqa";
