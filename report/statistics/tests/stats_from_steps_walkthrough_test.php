...
global $CFG;
require_once($CFG->dirroot . '/mod/realtimequiz/tests/attempt_walkthrough_from_csv_test.php');
 ...
 *
 * The realtimequiz stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-realtimequiz-tools/tree/master/statsspreadsheet
 *
 ...
 */
class stats_from_steps_walkthrough_test extends \mod_realtimequiz\attempt_walkthrough_from_csv_test {

    /**
     * @var realtimequiz_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    /**
     * Create a realtimequiz add questions to it, walk through realtimequiz attempts and then check results.
     *
    ...
    public function test_walkthrough_from_csv($realtimequizsettings, $csvdata) {
        ...
        list($questions, $realtimequizstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata,
                            $whichattempts, $whichtries, $groupstudentsjoins);
        if ($realtimequizsettings['testnumber'] === '00') {
            $this->check_variants_count_for_realtimequiz_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_realtimequiz_stats_for_realtimequiz_00($realtimequizstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_realtimequiz_00($questions, $questionstats, $whichtries, $qubaids) {
    ...
    /**
     * @param $realtimequizstats
     */
    protected function check_realtimequiz_stats_for_realtimequiz_00($realtimequizstats) {
        $realtimequizstatsexpected = [
            'median'             => 4.5,
        ...
        foreach ($realtimequizstatsexpected as $statname => $statvalue) {
            $this->assertEqualsWithDelta($statvalue, $realtimequizstats->$statname, abs($statvalue) * 1.5e-5, $realtimequizstats->$statname);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $realtimequizstats, 2 => $questionstats, 3 => $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries,
        ...
        $this->report = new realtimequiz_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->realtimequiz);
        list($realtimequizstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->realtimequiz,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudentsjoins,
                                                                                     $questions);

        $qubaids = realtimequiz_statistics_qubaids_condition($this->realtimequiz->id, $groupstudentsjoins, $whichattempts);

        // We will create some realtimequiz and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $realtimequizcalc = new calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($realtimequizcalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return [$questions, $realtimequizstats, $questionstats, $qubaids];
        }
        return [$questions, $realtimequizstats, $questionstats, $qubaids];
    }
