...
/**
 * Quiz statistics report, table for showing statistics of each question in the realtimequiz.
 *
...
/**
 * This table has one row for each question in the realtimequiz, with sub-rows when
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class realtimequiz_statistics_table extends flexible_table {
    /** @var stdClass the realtimequiz settings. */
    protected $realtimequiz;

    /** @var integer the realtimequiz course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-realtimequiz-report-statistics-report');
    }
     ...
     * @param int $cmid the realtimequiz course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($realtimequiz, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'realtimequiz_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'realtimequiz_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'realtimequiz');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'realtimequiz_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'realtimequiz_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'realtimequiz_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'realtimequiz_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'realtimequiz_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'realtimequiz_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'realtimequiz_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'realtimequiz_statistics');
    ...
    protected function col_actions($questionstat) {
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            return realtimequiz_question_action_icons($this->realtimequiz, $this->cmid,
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'realtimequiz_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, ['title' => get_string('detailedanalysis', 'realtimequiz_statistics')]);
                ...
                if ($this->is_calculated_question_summary($questionstat)) {
                    ...
                    // in the UI.
                    $name = html_writer::div(get_string('rangeofvalues', 'realtimequiz_statistics'));
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              ['title' => get_string('detailedanalysis', 'realtimequiz_statistics')]);
                }
            }
        }
        ...
        if ($this->is_calculated_question_summary($questionstat)) {
            $name .= html_writer::link($url, get_string('viewanalysis', 'realtimequiz_statistics'));
        } else if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'realtimequiz_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the realtimequiz. That is, the indended influence this question
    ...
    protected function col_intended_weight($questionstat) {
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            if (is_null($min) && is_null($max)) {
                ...
                $min = realtimequiz_report_scale_summarks_as_percentage($min, $this->realtimequiz);
                $max = realtimequiz_report_scale_summarks_as_percentage($max, $this->realtimequiz);
                return $this->format_range($min, $max);
            }
        } else {
            return realtimequiz_report_scale_summarks_as_percentage($questionstat->maxmark, $this->realtimequiz);
        }
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($this->is_calculated_question_summary($questionstat)) {
            ...
            if (is_null($min) && is_null($max)) {
                ...
                if ($negcovar) {
                    $min = get_string('negcovar', 'realtimequiz_statistics');
                }
            ...
            $negcovar = get_string('negcovar', 'realtimequiz_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'realtimequiz_statistics'),
                        ['class' => 'negcovar']);
            }
     ...
     * other questions in this realtimequiz.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    ...
    protected function format_range(string $min = null, string $max = null) {
        if (is_null($min) && is_null($max)) {
            ...
            return get_string('rangebetween', 'realtimequiz_statistics', $a);
        }
    }
