...
/**
 * Upgrade script for the realtimequiz module.
 *
...
function xmldb_realtimequiz_upgrade($oldversion) {
    ...
    if ($oldversion < 2020061501) {
        ...
        // Define field completionminattempts to be added to realtimequiz.
        ...
        upgrade_mod_savepoint(true, 2020061501, 'realtimequiz');
    }
    ...
    if ($oldversion < 2021052503) {
        ...
        if ($dbman->field_exists($table, $field)) {
            $sql = "SELECT q.id, m.id as realtimequizid " .
                "FROM {realtimequiz} q " .
            ...
            $records = $DB->get_recordset_sql($sql, ['name' => 'realtimequiz', 'completionpass' => 1], 0, 1000);
            while ($records->valid()) {
                $realtimequizmodule = null;
                foreach ($records as $record) {
                    $ids[] = $record->id;
                    $realtimequizmodule = $record->realtimequizid;
                }

                if ($ids) {
                    list($insql, $params) = $DB->get_in_or_equal($ids, SQL_PARAMS_NAMED);
                    $DB->set_field_select('course_modules', 'completionpassgrade', 1,
                        "module = :realtimequiz AND instance $insql", $params + ['realtimequiz' => $realtimequizmodule]);

                    // Reset the value so it doesn't get picked on the next run. The field will be dropped later.
                    $DB->set_field_select('realtimequiz', 'completionpass', 0, "id $insql", $params);
        ...
        upgrade_mod_savepoint(true, 2021052503, 'realtimequiz');
    }

    if ($oldversion < 2021101900) {

        // Define field gradednotificationsenttime to be added to realtimequiz_attempts.
        $table = new xmldb_table('realtimequiz_attempts');
        ...
        if (!$dbman->field_exists($table, $field)) {
            ...
            $DB->execute('UPDATE {realtimequiz_attempts} SET gradednotificationsenttime = timefinish');
        }

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2021101900, 'realtimequiz');
    }

    if ($oldversion < 2022020300) {
        // Define table realtimequiz_slot_tags to be dropped.
        $table = new xmldb_table('realtimequiz_slot_tags');

        // Conditionally launch drop table for realtimequiz_slot_tags.
        if ($dbman->table_exists($table)) {
            $dbman->drop_table($table);
        }

        // Define fields to be dropped from realtimequiz_slots.
        $table = new xmldb_table('realtimequiz_slots');

        // Define key questionid (foreign) to be dropped form realtimequiz_slots.
        $key = new xmldb_key('questionid', XMLDB_KEY_FOREIGN, ['questionid'], 'question', ['id']);

        // Launch drop key questionid.
        $dbman->drop_key($table, $key);

        // Define key questioncategoryid (foreign) to be dropped form realtimequiz_slots.
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2022020300, 'realtimequiz');
    }
    ...
    if ($oldversion < 2022120500) {
        // Define field displaynumber to be added to realtimequiz_slots.
        $table = new xmldb_table('realtimequiz_slots');
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2022120500, 'realtimequiz');
    }
