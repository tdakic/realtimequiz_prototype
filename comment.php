...
$PAGE->set_url('/mod/realtimequiz/comment.php', ['attempt' => $attemptid, 'slot' => $slot]);
...
if (!$attemptobj->is_finished()) {
    throw new \moodle_exception('attemptclosed', 'realtimequiz');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/realtimequiz:grade');

// Print the page header.
$PAGE->set_pagelayout('popup');
$PAGE->set_title(get_string('manualgradequestion', 'realtimequiz', [
        ...
        <div class="fitem fitem_actionbuttons fitem_fsubmit mt-3">
            <fieldset class="felement fsubmit">
                <input id="id_submitbutton" type="submit" name="submit" class="btn btn-primary" value="<?php
                        print_string('save', 'realtimequiz'); ?>"/>
            </fieldset>
        </div>
    </div>
</fieldset>
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_realtimequiz.init_comment_popup', null, false, realtimequiz_get_js_module());
