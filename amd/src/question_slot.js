...
/**
 * Render the question slot template for each question in the realtimequiz edit view.
 *
 * @module     mod_realtimequiz/question_slot
...
const setQuestionVersion = (slotId, newVersion) => fetchMany([{
    methodname: 'mod_realtimequiz_set_question_version',
    args: {
        slotid: slotId,
...
const registerEventListeners = () => {
    document.addEventListener('change', e => {
        if (!e.target.matches('[data-action="mod_realtimequiz-select_slot"][data-slot-id]')) {
            return;
        }
