...
namespace realtimequizaccess_openclosedate;

use mod_realtimequiz\realtimequiz_settings;
use realtimequizaccess_openclosedate;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/mod/realtimequiz/accessrule/openclosedate/rule.php');


/**
 * Unit tests for the realtimequizaccess_openclosedate plugin.
 *
...
class rule_test extends \basic_testcase {
    public function test_no_dates() {
        ...
        $realtimequiz->overduehandling = 'autosubmit';
        $cm = new \stdClass();
        $cm->id = 0;
        ...
        $rule = new realtimequizaccess_openclosedate($realtimequizobj, 10000);
        ...
        $rule = new realtimequizaccess_openclosedate($realtimequizobj, 0);
        $this->assertFalse($rule->prevent_access());
        $this->assertFalse($rule->prevent_new_attempt(0, $attempt));
        $this->assertFalse($rule->is_finished(0, $attempt));
        $this->assertFalse($rule->end_time($attempt));
    ...
    public function test_start_date() {
        ...
        $rule = new realtimequizaccess_openclosedate($realtimequizobj, 9999);
        $this->assertEquals($rule->prevent_access(),
            get_string('notavailable', 'realtimequizaccess_openclosedate'));
        $this->assertFalse($rule->prevent_new_attempt(0, $attempt));
        $this->assertFalse($rule->is_finished(0, $attempt));
        $this->assertFalse($rule->end_time($attempt));
        $this->assertFalse($rule->time_left_display($attempt, 0));
    ...
    public function test_close_date() {
        ...
        $rule = new realtimequizaccess_openclosedate($realtimequizobj, 20000);
        $this->assertFalse($rule->prevent_access());
        $this->assertFalse($rule->prevent_new_attempt(0, $attempt));
        $this->assertFalse($rule->is_finished(0, $attempt));
        ...
        $rule = new realtimequizaccess_openclosedate($realtimequizobj, 20001);
        $this->assertEquals($rule->prevent_access(),
            get_string('notavailable', 'realtimequizaccess_openclosedate'));
        $this->assertFalse($rule->prevent_new_attempt(0, $attempt));
    ...
    public function test_close_date_with_overdue() {
        ...
        $rule = new realtimequizaccess_openclosedate($realtimequizobj, 21000);
        $this->assertFalse($rule->prevent_access());

        $rule = new realtimequizaccess_openclosedate($realtimequizobj, 21001);
        $this->assertEquals($rule->prevent_access(),
                get_string('notavailable', 'realtimequizaccess_openclosedate'));
    }
}
