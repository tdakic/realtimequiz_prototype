 ...
 *
 * @module     realtimequizaccess_seb/validate_realtimequiz_access
...
import * as View from 'realtimequizaccess_seb/view';
 ...
 *
 * @param {init} cmid Value of course module id of the realtimequiz.
 * @param {boolean} autoreconfigure Value of Moodle setting: realtimequizaccess_seb/autoreconfigureseb.
 */
const safeExamBrowserKeysUpdated = (cmid, autoreconfigure = false) => {
    // Action opening up the realtimequiz.
    isQuizAccessValid(cmid).then((response) => {
        ...
        if (response.configkey && response.browserexamkey) {
            ...
            // If autoreconfigureseb is enabled, attempt to reconfigure page with realtimequiz settings.
...
const isQuizAccessValid = (cmid) => {
    const request = {
        methodname: 'realtimequizaccess_seb_validate_realtimequiz_keys',
        args: {
            cmid: cmid,
...
/**
 * Reload Safe Exam Browser with current realtimequiz configuration.
 *
 * @param {init} cmid Value of course module id of the realtimequiz.
 */
const reconfigureSafeExamBrowser = (cmid) => {
    const domain = Config.wwwroot.replace(/^http/i, 'seb');
    const redirecturl = domain + '/mod/realtimequiz/accessrule/seb/config.php?cmid=' + cmid;
