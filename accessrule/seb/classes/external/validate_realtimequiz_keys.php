 ...
 */
class validate_realtimequiz_keys extends external_api {
    ...
    public static function execute(string $cmid, string $url, ?string $configkey = null, ?string $browserexamkey = null): array {
        ...
        if (empty($configkey) && empty($browserexamkey)) {
            throw new invalid_parameter_exception(get_string('error:ws:nokeyprovided', 'realtimequizaccess_seb'));
        }

        // Check realtimequiz exists corresponding to cmid.
        if (($realtimequizid = self::get_realtimequiz_id($cmid)) === 0) {
            throw new invalid_parameter_exception(get_string('error:ws:realtimequiznotexists', 'realtimequizaccess_seb', $cmid));
        }

        $result = ['configkey' => true, 'browserexamkey' => true];

        $accessmanager = new seb_access_manager(realtimequiz_settings::create($realtimequizid));

        // Check if there is a valid config key.
        if (!$accessmanager->validate_config_key($configkey, $url)) {
            access_prevented::create_strict($accessmanager, get_string('invalid_config_key', 'realtimequizaccess_seb'),
        ...
        if (!$accessmanager->validate_browser_exam_key($browserexamkey, $url)) {
            access_prevented::create_strict($accessmanager, get_string('invalid_browser_key', 'realtimequizaccess_seb'),
                    $configkey, $browserexamkey)->trigger();
    ...
    /**
     * Check if there is a valid realtimequiz corresponding to a course module it.
     *
     * @param string $cmid Course module ID.
     * @return int Returns realtimequiz id if cmid matches valid realtimequiz, or 0 if there is no match.
     */
    private static function get_realtimequiz_id(string $cmid): int {
        $realtimequizid = 0;

        $coursemodule = get_coursemodule_from_id('realtimequiz', $cmid);
        if (!empty($coursemodule)) {
            $realtimequizid = $coursemodule->instance;
        }

        return $realtimequizid;
    }
}
