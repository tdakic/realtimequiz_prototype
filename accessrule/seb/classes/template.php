...
class template extends persistent {
    ...
    const TABLE = 'rtqaccess_seb_template';
    ...
    protected function validate_content(string $content) {
        if (helper::is_valid_seb_config($content)) {
            ...
            return new \lang_string('invalidtemplate', 'realtimequizaccess_seb');
        }
    }
    ...
    public function can_delete() : bool {
        ...
        if ($this->get('id')) {
            $settings = seb_realtimequiz_settings::get_records(['templateid' => $this->get('id')]);
