...
/**
 * Entity model representing realtimequiz settings for the seb plugin.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class seb_realtimequiz_settings extends persistent {

    /** Table name for the persistent. */
    const TABLE = 'rtqaccess_seb_rtqsettings';
    ...
    /**
     * Return an instance by realtimequiz id.
     *
     * This method gets data from cache before doing any DB calls.
     *
     * @param int $realtimequizid Quiz id.
     * @return false|\realtimequizaccess_seb\seb_realtimequiz_settings
     */
    public static function get_by_realtimequiz_id(int $realtimequizid) {
        if ($data = self::get_realtimequiz_settings_cache()->get($realtimequizid)) {
            return new static(0, $data);
        }

        return self::get_record(['realtimequizid' => $realtimequizid]);
    }

    /**
     * Return cached SEB config represented as a string by realtimequiz ID.
     *
     * @param int $realtimequizid Quiz id.
     * @return string|null
     */
    public static function get_config_by_realtimequiz_id(int $realtimequizid) : ?string {
        $config = self::get_config_cache()->get($realtimequizid);
        ...
        $config = null;
        if ($settings = self::get_by_realtimequiz_id($realtimequizid)) {
            $config = $settings->get_config();
            self::get_config_cache()->set($realtimequizid, $config);
        }
    ...
    /**
     * Return cached SEB config key by realtimequiz ID.
     *
     * @param int $realtimequizid Quiz id.
     * @return string|null
     */
    public static function get_config_key_by_realtimequiz_id(int $realtimequizid) : ?string {
        $configkey = self::get_config_key_cache()->get($realtimequizid);
        ...
        if ($settings = self::get_by_realtimequiz_id($realtimequizid)) {
            ...
            self::get_config_key_cache()->set($realtimequizid, $configkey);
        }
    ...
    private static function get_config_key_cache() : \cache_application {
        return \cache::make('realtimequizaccess_seb', 'configkey');
    }
    ...
    private static function get_config_cache() : \cache_application {
        return \cache::make('realtimequizaccess_seb', 'config');
    }

    /**
     * Return realtimequiz settings cache object,
     *
     * @return \cache_application
     */
    private static function get_realtimequiz_settings_cache() : \cache_application {
        return \cache::make('realtimequizaccess_seb', 'realtimequizsettings');
    }
    ...
    private function after_save() {
        self::get_realtimequiz_settings_cache()->set($this->get('realtimequizid'), $this->to_record());
        self::get_config_cache()->set($this->get('realtimequizid'), $this->config);
        self::get_config_key_cache()->set($this->get('realtimequizid'), $this->configkey);
    }
    ...
    protected function before_delete() {
        $key = $this->get('realtimequizid');
        self::get_realtimequiz_settings_cache()->delete($key);
    ...
    protected function validate_allowedbrowserexamkeys($keys) {
        ...
        foreach ($keys as $i => $key) {
            if (!preg_match('~^[a-f0-9]{64}$~', $key)) {
                return new lang_string('allowedbrowserkeyssyntax', 'realtimequizaccess_seb');
            }
        }
        if (count($keys) != count(array_unique($keys))) {
            return new lang_string('allowedbrowserkeysdistinct', 'realtimequizaccess_seb');
        }
        return true;
    }
    ...
    /**
     * Create or update the config string based on the current realtimequiz settings.
     */
    ...
    /**
     * Case for USE_SEB_UPLOAD_CONFIG. This creates a plist from an uploaded configuration file, then applies the realtimequiz
    ...
    private function process_seb_upload_config() {
        ...
        if (empty($file)) {
            throw new moodle_exception('noconfigfilefound', 'realtimequizaccess_seb', '', $this->get('cmid'));
        } else {
    ...
    private function process_required_enforced_settings() {
        ...
        $realtimequizurl = new moodle_url($CFG->wwwroot . "/mod/realtimequiz/view.php", ['id' => $this->get('cmid')]);
        $this->plist->set_or_update_value('startURL', new CFString($realtimequizurl->out(true)));
    ...
    /**
     * Sets the quitURL if found in the seb_realtimequiz_settings.
     */
    ...
    /**
     * Sets the realtimequiz_setting's linkquitseb if a quitURL value was found in a template or uploaded config.
     */
