...
namespace realtimequizaccess_seb\event;
...
class events_test extends \advanced_testcase {
     ...
     *
     * @covers \realtimequizaccess_seb\event\access_prevented
     */
    public function test_event_access_prevented() {
        $this->resetAfterTest();

        $this->setAdminUser();
        $realtimequiz = $this->create_test_realtimequiz($this->course, \realtimequizaccess_seb\settings_provider::USE_SEB_CONFIG_MANUALLY);
        $accessmanager = new \realtimequizaccess_seb\seb_access_manager(new realtimequiz_settings($realtimequiz,
            get_coursemodule_from_id('realtimequiz', $realtimequiz->cmid), $this->course));
        ...
        $event = \realtimequizaccess_seb\event\access_prevented::create_strict($accessmanager, 'Because I said so.');
        ...
        $this->assertEquals(
            "The user with id '$user->id' has been prevented from accessing realtimequiz with id '$realtimequiz->id' by the "
        ...
        $this->assertEquals($realtimequiz->id, $event->objectid);
     ...
     *
     * @covers \realtimequizaccess_seb\event\template_created
     */
    public function test_event_create_template() {
        ...
        $event = \realtimequizaccess_seb\event\template_created::create_strict(
            $template,
        ...
        // Test that the event data is as expected.
        $this->assertInstanceOf('\realtimequizaccess_seb\event\template_created', $event);
