@javascript @mod_realtimequiz @realtimequizaccess @realtimequizaccess_seb
Feature: Safe Exam Browser settings in realtimequiz edit form
  ...
  Scenario: Quiz setting "Require the use of Safe Exam Browser" has all types if at least one template has been added.
    Given the following "realtimequizaccess_seb > seb templates" exist:
      | name       |
      | Template 1 |
    When I am on the "Quiz 1" "realtimequiz activity editing" page logged in as admin
    And I expand all fieldsets
  ...
  Scenario: Quiz can be edited without capability to select SEB template
    Given the following "permission override" exists:
      | role         | editingteacher                       |
      | capability   | realtimequizaccess/seb:manage_seb_templateid |
      | permission   | Prevent                              |
      | contextlevel | System                               |
      | reference    |                                      |
    ...
    # Create the realtimequiz.
    When I add a "Quiz" to section "0" and I fill the form with:
      | Name | My realtimequiz |
    Then I should not see "Adding a new Quiz"
    # Edit the realtimequiz.
    And I open "My realtimequiz" actions menu
    And I click on "Edit settings" "link" in the "My realtimequiz" activity
    And I set the field "Name" to "My realtimequiz edited"
  ...
  Scenario: SEB settings if using No SEB
    Given the following "realtimequizaccess_seb > seb templates" exist:
      | name       |
      | Template 1 |
    And I am on the "Quiz 1" "realtimequiz activity editing" page logged in as admin
    And I expand all fieldsets
