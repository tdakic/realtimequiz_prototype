...
class settings_provider_test extends \advanced_testcase {
    ...
    /**
     * Mocked realtimequiz form instance.
     * @var \mod_realtimequiz_mod_form
     */
    protected $mockedrealtimequizform;
    ...
    protected function set_up_form_mocks() {
        ...
        $this->mockedrealtimequizform = $this->createMock('mod_realtimequiz_mod_form');
        $this->mockedrealtimequizform->method('get_context')->willReturn($this->context);
        $this->mockedrealtimequizform->method('get_instance')->willReturn($this->realtimequiz->id);
    ...
    /**
     * Test that settings types to be added to realtimequiz settings, are part of realtimequiz_settings persistent class.
     */
    public function test_setting_elements_are_part_of_realtimequiz_settings_table() {
        $dbsettings = (array) (new seb_realtimequiz_settings())->to_record();
        ...
        // Get all elements to be added to form, that are not in the persistent realtimequiz_settings class.
    ...
    public function test_hideifs() {
        $settinghideifs = settings_provider::get_realtimequiz_hideifs();
    ...
    public function test_build_setting_capability_name_incorrect_setting() {
        ...
        $this->expectExceptionMessage('Incorrect SEB realtimequiz setting broken');
    ...
    public function test_build_setting_capability_name_correct_setting() {
        foreach (settings_provider::get_seb_config_elements() as $name => $type) {
            $expected = 'realtimequizaccess/seb:manage_' . $name;
    ...
    public function test_get_requiresafeexambrowser_options($settingcapability) {
        ...
        assign_capability('realtimequizaccess/seb:manage_seb_templateid', CAP_ALLOW, $this->roleid, $this->context->id);
        $options = settings_provider::get_requiresafeexambrowser_options($this->context);
        $this->assertCount(4, $options);
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_CONFIG_MANUALLY, $options));
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_TEMPLATE, $options));
        ...
        assign_capability('realtimequizaccess/seb:manage_filemanager_sebconfigfile', CAP_ALLOW, $this->roleid, $this->context->id);
        $options = settings_provider::get_requiresafeexambrowser_options($this->context);
        $this->assertCount(5, $options);
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_CONFIG_MANUALLY, $options));
        $this->assertTrue(array_key_exists(settings_provider::USE_SEB_TEMPLATE, $options));
    ...
    public function test_form_elements_are_frozen_if_conflicting_permissions() {
        ...
        assign_capability('realtimequizaccess/seb:manage_seb_requiresafeexambrowser', CAP_ALLOW, $this->roleid, $this->context->id);
        assign_capability('realtimequizaccess/seb:manage_seb_showsebdownloadlink', CAP_ALLOW, $this->roleid, $this->context->id);
        assign_capability('realtimequizaccess/seb:manage_seb_allowedbrowserexamkeys', CAP_ALLOW, $this->roleid, $this->context->id);

        $this->set_up_form_mocks();

        settings_provider::add_seb_settings_fields($this->mockedrealtimequizform, $this->mockedform);
    ...
    /**
     * Test that All settings are frozen if realtimequiz was attempted and use seb with manual settings.
     */
    public function test_form_elements_are_locked_when_realtimequiz_attempted_manual() {
        $this->resetAfterTest();
        $this->course = $this->getDataGenerator()->create_course();
    ...
    /**
     * Test that All settings are frozen if a realtimequiz was attempted and use template.
     */
    public function test_form_elements_are_locked_when_realtimequiz_attempted_template() {
        $this->resetAfterTest();
        $this->setAdminUser();
        $this->course = $this->getDataGenerator()->create_course();
    ...
    public function test_validate_draftarea_configfile_failure() {
        ...
        $itemid = $this->create_test_draftarea_file($xml);
        $errors = settings_provider::validate_draftarea_configfile($itemid);
        $this->assertEquals($errors, new \lang_string('fileparsefailed', 'realtimequizaccess_seb'));
    }
    ...
    /**
     * Test saving files from the user draft area into the realtimequiz context area storage.
     */
    public function test_save_filemanager_sebconfigfile_draftarea() {
        ...
        settings_provider::save_filemanager_sebconfigfile_draftarea($draftitemid, $this->realtimequiz->cmid);

        $fs = get_file_storage();
        $files = $fs->get_area_files($this->context->id, 'realtimequizaccess_seb', 'filemanager_sebconfigfile');

        $this->assertCount(2, $files);
    }

    /**
     * Test deleting the $this->realtimequiz->cmid itemid from the file area.
     */
    public function test_delete_uploaded_config_file() {
        ...
        settings_provider::delete_uploaded_config_file($this->realtimequiz->cmid);
    ...
    public function test_get_module_context_sebconfig_file() {
        ...
        $file = settings_provider::get_module_context_sebconfig_file($this->realtimequiz->cmid);
    ...
    public function test_is_seb_settings_locked() {
        ...
        $this->assertFalse(settings_provider::is_seb_settings_locked($this->realtimequiz->id));

        $this->attempt_realtimequiz($this->realtimequiz, $user);
        $this->assertTrue(settings_provider::is_seb_settings_locked($this->realtimequiz->id));
    }
