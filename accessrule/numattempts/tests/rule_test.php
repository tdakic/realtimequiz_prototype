...
namespace realtimequizaccess_numattempts;

use mod_realtimequiz\realtimequiz_settings;
use realtimequizaccess_numattempts;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/mod/realtimequiz/accessrule/numattempts/rule.php');


/**
 * Unit tests for the realtimequizaccess_numattempts plugin.
 *
...
class rule_test extends \basic_testcase {
    public function test_num_attempts_access_rule() {
        ...
        $rule = new realtimequizaccess_numattempts($realtimequizobj, 0);
        $attempt = new \stdClass();

        $this->assertEquals($rule->description(),
            get_string('attemptsallowedn', 'realtimequizaccess_numattempts', 3));

        $this->assertFalse($rule->prevent_new_attempt(0, $attempt));
        $this->assertFalse($rule->prevent_new_attempt(2, $attempt));
        $this->assertEquals($rule->prevent_new_attempt(3, $attempt),
            get_string('nomoreattempts', 'realtimequiz'));
