...
namespace realtimequizaccess_password;

use mod_realtimequiz\realtimequiz_settings;
use realtimequizaccess_password;

defined('MOODLE_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/mod/realtimequiz/accessrule/password/rule.php');


/**
 * Unit tests for the realtimequizaccess_password plugin.
 *
...
class rule_test extends \basic_testcase {
    public function test_password_access_rule() {
        ...
        $realtimequiz->password = 'frog';
        $cm = new \stdClass();
        $cm->id = 0;
        $realtimequizobj = new realtimequiz_settings($realtimequiz, $cm, null);
        $rule = new realtimequizaccess_password($realtimequizobj, 0);
